...
export function buildOptionTree({ colours = [], hardware = [], rooms = [], panelUses = [] } = {}) {
  ...
  return {
    ...
    brands,
    types: [...PRODUCT_TYPES],
    // What a made to measure row can be: everything that is cut to a size out
    // of the colour library, fronts and a plain laminate sheet alike. Hardware
    // is its own tab now, with its own columns, so offering it here would be a
    // row that then wants a board, a size and a colour it has not got.
