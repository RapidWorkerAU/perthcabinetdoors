...
export function settlementMethodLabel(key) {
  return SETTLEMENT_METHODS.find((entry) => entry.key === key)?.label || "Some other way";
}

export function settlementWantsReference(key) {
 ...
 */
export function settlementPatch(payment, { method, reference = "", paidAt = "", note = "" } = {}, today = new Date()) {
  ...
  if (!SETTLEMENT_METHOD_KEYS.includes(method)) {
