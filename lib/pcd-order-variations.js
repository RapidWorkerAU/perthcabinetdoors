...
import { CHANGEABLE_COLUMNS } from "./pcd-variation-override";
...
export function orderLineFromVariationLine(line, orderId, variationId, sortOrder) {
  return {
    ...
    qty: line.qty || 1,
    // WHAT THE WORKSHOP NEEDS TO MAKE IT. A variation line reaches the floor
    // the same way an original one does, and until now it carried nothing at
    // all about drilling: the label printed "Not recorded" and somebody had
    // to go and ask. A door that needed drilling and did not get it is scrap.
    cabinet_brand: line.cabinet_brand || null,
    hinge_holes: line.hinge_holes ?? null,
    hinge_qty: line.hinge_qty || null,
    ...
    // The kind of panel, the edges, the boring, the grain and who supplies it,
    // where the variation line has them. See lineAnswers.
    ...lineAnswers(line),
...
/**
 * Write an approved variation onto the order it belongs to.
 *
 * holdPricing is the admin override path, and it is the ONLY thing that behaves
 * differently here. With it set, the order's money is not touched at all: the
 * changed line keeps the price, the costs and the markup it already had, and
 * the order's subtotal, GST, total and job costs are left exactly as they were.
 *
 * That is not a discount applied to the order; it is the order not moving. It
 * is safe precisely because the override refuses any variation that could have
 * moved it: no adds, no removals, no quantity, no spec, no price adjustments,
 * no job costs, only a size. See lib/pcd-variation-override.js, which is where
 * that is decided, and which decides it BEFORE anything reaches this function.
 *
 * Holding the price is what keeps the order's line items summing to its
 * subtotal, which is what the tax invoice checks before it will print without
 * a warning.
 */
export async function applyAcceptedVariation(supabase, variationId, { actorType = "system", holdPricing = false } = {}) {
  const { data: variation, error: variationError } = await supabase
    .from("pcd_order_variations")
    .select("*, pcd_orders(*)")
    .eq("id", variationId)
  ...
  for (const line of lines || []) {
    ...
    if (line.action === "change" && line.order_line_item_id) {
      ...
      const { data: existingLine } = await supabase
        .from("pcd_order_line_items")
        // The colour comes with it, because a held apply has to know whether
        // the shade moved before it decides what to do with the cost source.
        .select("cabinet_config_snapshot, colour")
        .eq("id", line.order_line_item_id)
        .eq("order_id", order.id)
        .maybeSingle();
      const rebuiltCabinet = resnapshotCabinet(existingLine?.cabinet_config_snapshot, line);

      // THE OVERRIDE WRITES THE SIZE AND LEAVES EVERYTHING ELSE ALONE.
      //
      // Not the full patch with the price swapped out. The full patch would
      // rewrite the colour, the edge, the boring, the unit cost and the markup
      // from whatever the variation line happens to carry, and a variation line
      // carries a lot of fields nobody ever filled in. Under the override those
      // fields are already known to match, so rewriting them can only do harm,
      // and the costing columns must not move at all or the line stops agreeing
      // with the price it is keeping.
      //
      // So: the columns the override is allowed to change, the rebuilt panel
      // list that goes with a new size, the stamp saying which variation did
      // this, and the note. Nothing else.
      //
      // THE COLUMNS COME FROM THE GATE ITSELF. CHANGEABLE_COLUMNS is the same
      // list lib/pcd-variation-override.js uses to decide what qualifies, read
      // here rather than copied, because a field allowed through the gate and
      // not written here is a change that never reaches the workshop. That is
      // how a door gets cut in last month's colour with everything on screen
      // looking right.
      if (holdPricing) {
        const heldPatch = {
        ...
        for (const column of CHANGEABLE_COLUMNS) {
          heldPatch[column] = line[column] ?? null;
        }
        // THE COST SOURCE CANNOT SURVIVE A COLOUR CHANGE.
        //
        // unit_cost_source_id points at the colour library row the price came
        // from. Leave it pointing at the old shade and the line says one board
        // and its id says another, which is the exact shape of the fault that
        // put a wardrobe on a quote at the wrong rate. The price itself is
        // held, so nulling this changes no money today; it only stops a later
        // reprice trusting a row that contradicts the words beside it. See the
        // note on matchBoardCost in lib/pcd-board-cost.js.
        if (String(line.colour ?? "") !== String(existingLine?.colour ?? "")) {
          heldPatch.unit_cost_source_id = null;
          heldPatch.unit_cost_source_label = null;
        }
      ...
      const changePatch = {
          ...
          line_total_ex_gst: toNumber(line.proposed_line_total_ex_gst),
          notes: line.notes || null,
          // Only the answers this variation line actually carries, so changing
          // a door's size does not wipe its banded edges. See lineAnswers.
          ...lineAnswers(line),
      };
  ...
  // WHERE THE OVERRIDE STOPS THE MONEY.
  //
  // Held: the order keeps the figures it already had, down to the cent, and no
  // update is sent at all. Not "the same numbers written again": nothing is
  // written, so there is no chance of a rounding pass or a missing column
  // quietly moving a total that was supposed to stand still.
  //
  // The totals are still worked out, because the activity log below says what
  // the change would have come to and that is the number somebody wants when
  // they ask later what was given away.
  const subtotal = holdPricing
    ? roundMoney(toNumber(order.subtotal_ex_gst))
    : roundMoney(toNumber(order.subtotal_ex_gst) + toNumber(variation.subtotal_ex_gst));
  const gst = holdPricing
    ? roundMoney(toNumber(order.gst_amount))
    : roundMoney(toNumber(order.gst_amount) + toNumber(variation.gst_amount));
  const total = holdPricing
    ? roundMoney(toNumber(order.total_inc_gst))
    : roundMoney(toNumber(order.total_inc_gst) + toNumber(variation.total_inc_gst));
  ...
  let { error: orderError } = holdPricing
    ? { error: null }
    : await supabase.from("pcd_orders").update(orderPatch).eq("id", order.id);
  ...
  // EVERY OTHER VARIATION STILL OUT HAS TO BE TOLD THE ORDER MOVED.
  //
  // recalcVariation works out "your order will come to X" as the order's total
  // plus that variation's own delta, and stores it as revised_order_total_inc_gst.
  // It is printed on the page the customer opens.
  //
  // Nothing stops two variations being out at once, and applying this one has
  // just changed the total the other was worked out against. Left alone, the
  // second variation goes on promising a figure that is short by the whole of
  // this one, and the customer approves a number we will not honour.
  //
  // Draft ones are included on purpose: the order total is what the staff member
  // is looking at while they price it, and a stale figure there becomes a stale
  // promise the moment they press Send.
  //
  // Never throws. This variation has been applied and the order is right; a
  // failure to refresh a sibling is a wrong figure on a page, not a reason to
  // fail work the customer has already agreed to.
  try {
    const { data: stillOut } = await supabase
      .from("pcd_order_variations")
      .select("id")
      .eq("order_id", order.id)
      .neq("id", variationId)
      .in("status", ["draft", "sent", "viewed", "awaiting_deposit"]);
    for (const other of stillOut || []) {
      await recalcVariation(supabase, other.id);
    }
  } catch (recalcError) {
    console.error(
      `[variation] ${variation.variation_number} was applied, but the other open variations on this order could ` +
        "not be refreshed. They may be showing a revised order total that is now too low.",
      recalcError?.message || recalcError
    );
  }
  ...
  await logOrderActivity(supabase, {
    ...
    title: holdPricing ? "Variation applied to order at no charge" : "Variation applied to order",
    // The amount is still said out loud on a held apply, because "at no charge"
    // without a figure beside it is not a record of anything. It is what the
    // change WOULD have come to, and the order total below it did not move.
    description: holdPricing
      ? `${variation.variation_number} - ${formatMoney(variation.total_inc_gst, variation.currency || "AUD")} not charged`
      : `${variation.variation_number} - ${formatMoney(variation.total_inc_gst, variation.currency || "AUD")}`,
    metadata: {
