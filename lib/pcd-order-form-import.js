...
import { unprefixWorkbook } from "./pcd-xlsx-normalise";
...
import { sizeProblems } from "./pcd-size-limits";
import {
  EDGE_FINISHES,
...
export function lineWarnings(line, index, tabLabel = "") {
  ...
  // A SIZE WE CANNOT PRESS, CAUGHT AS THE FORM COMES BACK IN.
  // The spreadsheet itself cannot easily police this, because the limit depends
  // on the brand and the material chosen two columns away, so it is read here
  // where both are known. See lib/pcd-size-limits.js.
  sizeProblems(line).forEach((problem) => out.push(`${where}: ${problem}`));
 ...
 */
/**
 * WHAT KIND OF FILE THIS ACTUALLY IS, from its first few bytes.
 *
 * The extension is whatever somebody typed. The magic number is what the file
 * is, and the three wrong answers we see are each worth a different sentence:
 * an .xls saved before 2007, a .csv or a web page renamed, and a download that
 * arrived empty. "Could not be opened as a spreadsheet" for all three sends
 * somebody looking for a fault in the form.
 */
export function fileKind(buffer) {
  const bytes = buffer && buffer.length ? buffer : null;
  if (!bytes || bytes.length < 8) return "empty";
  const head = Buffer.isBuffer(bytes) ? bytes : Buffer.from(bytes);
  // PK.. — a zip, which is what every .xlsx is.
  if (head[0] === 0x50 && head[1] === 0x4b) return "zip";
  // The old OLE2 container: a real .xls, or a .doc renamed.
  if (head[0] === 0xd0 && head[1] === 0xcf && head[2] === 0x11 && head[3] === 0xe0) return "xls";
  return "other";
}

const OPEN_ERRORS = {
  empty: "That file is empty. The download may not have finished, so try downloading the order form again.",
  xls:
    "That is an old .xls workbook, or an Excel file with a password on it. Open it in Excel, take any password off, " +
    "and use Save As to save it as Excel Workbook (.xlsx).",
  other:
    "That is not an Excel workbook. A .csv or a saved web page cannot be read here: download the order form, " +
    "fill it in, and upload the .xlsx it saves as.",
};

/**
 * WHY A FILE THAT LOOKS LIKE AN .XLSX WILL NOT OPEN.
 *
 * Every .xlsx is a zip, so "it is a zip" is not the same as "it is a workbook
 * we can read". Three things get uploaded that pass the first test and fail the
 * second, and each of them is somebody one Save As away from success:
 *
 *   STRICT OPEN XML   sits directly under "Excel Workbook" in Excel's Save As
 *                     list, saves with a .xlsx extension, and uses a different
 *                     namespace that no reader outside Excel supports.
 *   AN .ODS OR SIMILAR  a zip with a spreadsheet in it, but not this one.
 *   A DAMAGED ZIP     a download that stopped early, which Excel offers to
 *                     repair and nothing else can read at all.
 *
 * Returns null when the file is a workbook we should be able to read, so the
 * parser's own message is what gets reported for anything rarer than these.
 */
export async function workbookOpenProblem(buffer) {
  let zip;
  try {
    zip = await JSZip.loadAsync(buffer);
  } catch {
    return "That file is damaged and cannot be opened. If Excel offers to repair it, let it, save it again, and upload that.";
  }

  if (!zip.file("xl/workbook.xml")) {
    if (zip.file("content.xml")) {
      return "That is an OpenDocument spreadsheet. Open it and use Save As to save it as Excel Workbook (.xlsx).";
    }
    return "That zip does not contain an Excel workbook. Please upload the .xlsx you filled in.";
  }

  const xml = await zip.file("xl/workbook.xml").async("string");
  if (xml.includes("purl.oclc.org/ooxml")) {
    return (
      "That was saved as Strict Open XML, which only Excel can read. In Excel use Save As and choose " +
      "\"Excel Workbook (.xlsx)\", the first one on the list, rather than \"Strict Open XML Spreadsheet\"."
    );
  }
  return null;
}
...
export async function readOrderForm(buffer, { mapping = {} } = {}) {
  ...
  // Said before it is parsed, because the parser's own message for a .csv is
  // about zip headers and helps nobody.
  const kind = fileKind(buffer);
  if (kind !== "zip") return { ok: false, error: OPEN_ERRORS[kind], ...nothing };
  const problem = await workbookOpenProblem(buffer);
  if (problem) return { ok: false, error: problem, ...nothing };

  // A WORKBOOK WRITTEN WITH NAMESPACE PREFIXES IS STILL A WORKBOOK. LibreOffice
  // and several online editors write one, Excel opens it without a murmur, and
  // our reader matched element names literally and parsed it to nothing. Put
  // back into the spelling the reader understands rather than refused: the
  // customer filled the form in correctly and the tool they used is not their
  // fault. See lib/pcd-xlsx-normalise.js.
  const readable = (await unprefixWorkbook(buffer)) || buffer;

  try {
    await workbook.xlsx.load(readable);
  } catch (error) {
    // THE REASON, NOT JUST THE FACT. This swallowed the parser's message
    // entirely, so a file that would not open could only ever be guessed at:
    // there was nothing to act on and nothing in the log either.
    const detail = String(error?.message || "").trim();
    console.error(`[order-form-import] xlsx.load failed on a ${buffer.length} byte file: ${detail}`);
    return {
      ok: false,
      error: detail
        ? `That file could not be opened as a spreadsheet. Excel reported: ${detail}`
