...
import { kickboardAllowedFor } from "./pcd-ikea-presets";
...
export const CABINET_MOUNT_MM = {
  ...
  wall_cabinet:  1400,
  floating_shelf: 1500,
  // A Shelf & Rail hangs so the TOP OF THE SHELF lands at 1800 — the classic AU
  // single-hang robe height. mount_height_mm is the underside of the assembly
  // like every other item, so this is 1800 less the standard 100mm cleat and
  // 18mm shelf. The form asks for (and shows) the shelf top; see
  // mountForShelfTopMm in pcd-shelf-rail-utils.
  shelf_rail:    1682,
  tall_cabinet:  0,
  corner_base_cabinet: 0,
  corner_tall_cabinet: 0,
...
// The corner (L-shaped) cabinet types — a base corner and (audit p2-5) a
// full-height corner pantry. Both share the same L geometry, bi-fold corner
// door and per-leg finished backs; the corner cut-list is driven by the
// is_corner config flag, and every geometry / render / import corner check
// uses this one set so the two types stay in step. Benchtops deliberately do
// NOT use this set — a tall corner has no benchtop.
export const CORNER_TYPES = new Set(["corner_base_cabinet", "corner_tall_cabinet"]);
export function isCornerType(item) {
  return CORNER_TYPES.has(item?.item_type ?? item);
}

// Items that render with the corner L / diagonal FOOTPRINT: the corner cabinets
// PLUS the decorative "brick corner pantry" room feature, which reuses the
// diagonal corner shape but is never quoted and has no cabinetry config. Use
// this ONLY in the geometry/rendering layer; the cut-list / import corner logic
// uses isCornerType (cabinets only) so the brick pantry never becomes a cabinet.
export function isCornerShaped(item) {
  return isCornerType(item) || (item?.item_type ?? item) === "brick_corner_pantry";
}
...
export function kickboardOffsetMm(item) {
  return kickboardIsInset(item) ? 0 : kickboardHeightMm(item);
}

// A bookcase carries its plinth INSIDE the carcass. Its sides run all the way
// to the floor — waterfall ends — and the kickboard is a rail set back BETWEEN
// them, so the carcass is never lifted, the height the user types is the true
// overall height, and the plinth can never merge into a continuous run with the
// cabinet beside it: it's a board inside this bookcase, not a strip across a row
// of them. Every other floor cabinet sits ON its kickboard, kitchen-style.
export const INSET_KICKBOARD_TYPES = new Set(["bookcase"]);
export function kickboardIsInset(item) {
  return INSET_KICKBOARD_TYPES.has(item?.item_type ?? item);
}

/**
 * Does this cabinet have a kickboard of OURS on it.
 *
 * has_kickboard on its own is not the answer, and reading it raw is how a Pax
 * ended up offering a kickboard colour: an IKEA frame that comes with its own
 * plinth can still carry the flag from an older design, or from a screen that
 * used to offer the toggle, and every list that asked the raw field believed it.
 *
 * A wall cabinet never has one either, which half the callers remembered and
 * half did not.
 *
 * Ask this wherever the question is "is there a kickboard here": the parts a
 * customer can colour, the cut list, the plan PDF, the summary lines. Use
 * kickboardHeightMm to draw or cut the board itself.
 */
export function hasKickboard(item) {
  return Boolean(item?.has_kickboard) && item?.item_type !== "wall_cabinet" && kickboardAllowedFor(item);
}

// The plinth board's own height, whichever construction it is. 0 when there
// isn't one. Use this to DRAW or CUT the board; use kickboardOffsetMm to find
// out how far the carcass above it has been lifted.
export function kickboardHeightMm(item) {
  if (!item?.has_kickboard || item.item_type === "wall_cabinet") return 0;
  // A frame that came with its own plinth never gets one of ours on top, even
  // if the flag is set on it from an older design or from a screen that offered
  // the toggle. The cut list already refused to make the board; the geometry did
  // not, so a Pax with the flag on was drawn standing 120mm off the floor, a
  // whole plinth taller than the wardrobe actually is.
  if (!kickboardAllowedFor(item)) return 0;
  return Number(item.kickboard_height_mm) || DEFAULT_KICKBOARD_HEIGHT_MM;
}

// The plinth board's finished length: for an inset plinth it spans only the gap
// between the two sides, not the cabinet's full along-wall width.
export function kickboardSpanMm(item) {
  if (!kickboardIsInset(item)) return wallSpanMm(item);
  const t = Number(item?.carcass_thickness_mm) || 16;
  return Math.max(0, wallSpanMm(item) - 2 * t);
}
...
function secondaryFootprint(item, room) {
  if (!isCornerType(item) || !item.secondary_wall || item.secondary_wall === item.wall) {
    return null;
  }
...
export function kickboardSegments(item, room) {
  ...
  if (!isCornerType(item)) {
    ...
    const length = kickboardSpanMm(item);
...
export function computeKickboardRun(item, allItems, room) {
  ...
  return {
    legs: mySegments.map((seg) => {
      // An inset plinth is internal to its own carcass — it never runs on into
      // the next cabinet, and no neighbour's kickboard runs into it.
      if (kickboardIsInset(item)) {
        return { leg: seg.leg, wall: seg.wall, firstItemId: item.id, firstLeg: seg.leg, totalWidth: seg.length, count: 1 };
      }
      const candidates = allItems
        .filter((i) => i.room_id === item.room_id && i.has_kickboard && !kickboardIsInset(i) && (i.kickboard_span || "continuous") === "continuous")
...
export function computeAllKickboardRuns(roomItems, room) {
  ...
  for (const item of roomItems) {
    if (!item.has_kickboard || (item.kickboard_span || "continuous") !== "continuous" || item.item_type === "wall_cabinet" || kickboardIsInset(item)) {
      continue;
    }
