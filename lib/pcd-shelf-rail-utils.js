...
// Cleats are 18mm unless somebody picks otherwise — they're the structural
// part, so they don't follow the SHELF down to 16mm just because the shelf
// board is 16.
//
// BUT A BOARD'S THICKNESS IS THE BOARD'S. Picking the cleats' board was
// restricted to library colours stocked in 18mm, which meant a robe done in a
// 16mm colour could not have its cleats in that colour at all: the only 18mm
// board on hand was the carcass, so the cleats came out grey in a black robe
// and priced off a different board. So any board can be chosen now, and when
// one is, the cleats are as thick as it is. 18 is where it starts, not a lock.
...
/**
 * How thick the cleats and the front rail are.
 *
 * The board somebody picked for them, or 18mm. Read straight off the stored
 * config rather than through shelfRailConfig, because shelfRailConfig calls
 * this to fill its own field in.
 */
export function cleatThicknessMm(item) {
  const picked = item?.shelf_rail_config?.cleat_style;
  const chosen = Number(picked?.thickness_mm);
  const hasBoard = picked && (picked.material || picked.colour);
  return hasBoard && Number.isFinite(chosen) && chosen > 0 ? chosen : CLEAT_THICKNESS_MM;
}

/** Whether the chosen cleat board is thinner than we would normally build in. */
export function cleatIsThin(item) {
  return cleatThicknessMm(item) < CLEAT_THICKNESS_MM;
}
...
export function shelfRailConfig(item) {
  ...
  return {
    ...
    // As thick as the board chosen for them, 18 when nobody has chosen one.
    // The front rail's setback is measured off this, so a 16mm cleat moves the
    // rail and the cut list with it.
    cleat_thickness_mm: cleatThicknessMm(item),
    front_rail: {
      on: fr.on !== false,
...
// The cleats' finish — their own if one was picked, otherwise the shelf's.
//
// WHICH LIBRARY ROW IT IS COMES TOO. Without it a cleat in the shelf's colour
// was looked up by name at 18mm, and where that colour is only stocked in 16mm
// there is no such board: the price then came from wherever the name landed.
// The row is the answer, so it travels with the rest.
export function cleatStyle(item) {
  const cfg = shelfRailConfig(item);
  const s = cfg.cleat_style;
  const picked = Boolean(s && (s.material || s.colour));
  const base = picked ? s : shelfRailStyle(item);
  return {
    material: base.material || "",
    finish: base.finish || "",
    colour: base.colour || "",
    // A picked board is as thick as it is. The shelf's colour, borrowed because
    // nobody picked, is still cut at the structural 18.
    thickness_mm: picked ? cleatThicknessMm(item) : CLEAT_THICKNESS_MM,
    cost_per_sqm: Number(base.cost_per_sqm) || Number(item?.cost_per_sqm_carcass) || 0,
    colour_library_id: base.colour_library_id || (picked ? null : item?.colour_library_id) || null,
    supplier: base.supplier || base.supplier_name || (picked ? "" : item?.supplier_name) || "",
  };
}
