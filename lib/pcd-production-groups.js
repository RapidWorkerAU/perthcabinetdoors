 ...
 *
 * The order line's OWN link wins. It used to be read back through the quote
 * line, which meant a line a variation added, having no quote line, could never
 * be grouped and printed loose forever. The read-back stays for orders raised
 * before the column existed.
 */
function designItemFor(item, quoteLinesById) {
  return item?.design_item_id || quoteLinesById.get(item?.quote_line_item_id)?.design_item_id || null;
}
...
export function groupProductionRows(rows = [], { items = [], quoteLines = [] } = {}) {
  ...
  items.forEach((item) => {
    const designItemId = designItemFor(item, quoteLinesById);
