...
import { readAccessories } from "./pcd-cabinet-accessories";
import { CABINET_MOUNT_MM } from "./pcd-kickboard-utils";
import { HOLE_TYPES, bandedEdgeList } from "./pcd-line-details";
import { PANEL_STYLE_DEFAULTS, panelStyleFromDefault } from "./pcd-panel-options";

// Which edges are taped: a clean list in the fixed order, or null when nobody
// has changed it (read as all four). Anything that is not a list is null.
function dbBandedEdges(value) {
  return Array.isArray(value) ? bandedEdgeList(value) : null;
}

// A boring we machine, or null.
function dbHoleType(value) {
  return HOLE_TYPES.includes(value) ? value : null;
}

// What is fitted inside the cabinet, cleaned to the stored shape. Read through
// the same function the panel and the quote read it with, so a hand-rolled
// payload cannot put a handle, a price or a stray field in the list.
function dbAccessories(value) {
  return readAccessories({ accessories: value });
}
...
export function applyMaterialDefaults(payload, defaults) {
  ...
  if (CABINET_TYPES.includes(payload.item_type)) {
    ...
    // The applied panels: a kickboard, a filler and the finished panels. Only
    // where the project actually set one, so a job that has never touched them
    // goes on cutting a kickboard from its cabinet's own carcass.
    for (const def of PANEL_STYLE_DEFAULTS) {
      if (!def.applies(payload)) continue;
      const style = panelStyleFromDefault(def, defaults[def.key]);
      if (style && blank(merged[def.styleKey])) merged[def.styleKey] = style;
      if (style?.thickness_mm && def.thicknessField && blank(merged[def.thicknessField])) {
        merged[def.thicknessField] = style.thickness_mm;
      }
    }
    ...
    // The cleats and the front rail, which live in the config blob. Only where
    // the project set one; otherwise they go on matching their own shelf.
    const cleat = defaults.shelf_rail_cleat;
    if (cleat && String(cleat.material || "").trim() && !merged.shelf_rail_config?.cleat_style) {
      merged.shelf_rail_config = { ...(merged.shelf_rail_config || {}), cleat_style: { ...cleat } };
    }
...
export function buildItemRow(payload, projectId) {
  ...
  return {
    ...
    // Marks a standard-size prop the customer already owns (IKEA). Set once, at
    // insert: it is deliberately absent from buildItemPatch, so nothing can
    // later flip a prop into something we would quote, or the other way round.
    preset_ref: dbText(payload.preset_ref),
    // Visual-only carcass finish for such a prop. Kept out of material/finish/
    // colour so finishing pieces can't inherit an IKEA finish. Never quoted.
    prop_carcass_finish: dbText(payload.prop_carcass_finish),
    ...
    ...(isObstruction
      ...
      ? {
          ...
          // Per-panel reach and profile, keyed by panel (pcd-panel-options.js).
          // Empty means every panel follows the two flags above.
          panel_options:          payload.panel_options ?? {},
          ...
          // What is fitted INSIDE the cabinet: rails, bins, lights. Bought
          // rather than cut, so each one becomes its own Hardware line, priced
          // from the library when the quote is staged. See
          // lib/pcd-cabinet-accessories.js.
          accessories:               dbAccessories(payload.accessories),
          ...
          // The kickboard and the filler follow the carcass unless the project
          // set a default for them, which applyMaterialDefaults puts here.
          kickboard_style:        payload.kickboard_style ?? null,
          filler_panel_style:     payload.filler_panel_style ?? null,
          end_left_style:         payload.end_left_style ?? null,
          // Which edges are taped on the fronts (a cabinet) or the piece itself
          // (a standalone panel or scribe), and the hinge boring for the doors.
          banded_edges:           dbBandedEdges(payload.banded_edges),
          hole_type:              dbHoleType(payload.hole_type),
          end_right_style:        payload.end_right_style ?? null,
...
// Which colour library row the carcass and the shelves were picked from, and
// whose board it is. A colour NAME is not unique across suppliers, so without
// these a carcass in a colour two brands both stock cannot be priced: the board
// resolver refuses to guess between two rows at two prices, which is right, but
// it means somebody has to pick the supplier by hand on the quote.
//
// Everything else in a design already records this, because fronts and panels
// keep their board in a JSON column and whatever the picker returns is stored
// whole. The carcass and the shelves are older and live in flat columns.
//
// Listed here so the write can be retried without them while the migration that
// adds them has not been run yet. A live design tool must not start refusing to
// save because of a column that is not there: the colour still lands, it just
// has to be matched back to the library by name until it is.
export const DESIGN_COLOUR_SOURCE_COLUMNS = [
  "colour_library_id",
  "supplier_name",
  "shelf_colour_library_id",
  "shelf_supplier_name",
  // Added by 202609111900_pcd_design_item_edges_and_hole_type.sql, on the
  // same list for the same reason: a save drops them rather than failing.
  "banded_edges",
  "hole_type",
  // Added by 202609131000_pcd_design_item_accessories.sql. Same reason again:
  // a cabinet still saves on a database that has not had it run, it simply
  // cannot remember its rails and bins until it does.
  "accessories",
];

export function withoutDesignColourSource(row) {
  const copy = { ...row };
  for (const column of DESIGN_COLOUR_SOURCE_COLUMNS) delete copy[column];
  return copy;
}

// PostgREST reports an unknown column as PGRST204 and names it. Same pattern as
// the quote request line saver's guard.
export function isMissingDesignColourSourceError(error) {
  const message = String(error?.message || "");
  return error?.code === "PGRST204" && DESIGN_COLOUR_SOURCE_COLUMNS.some((column) => message.includes(column));
}
...
export function buildItemPatch(payload) {
  ...
  // Changeable, unlike preset_ref: which IKEA finish their own cabinet is in is
  // theirs to correct. It paints the box and nothing else.
  str("prop_carcass_finish");
  ...
  // A uuid column: an empty string is not a valid one, so a cleared colour has
  // to write null rather than "".
  const uuid = (key) => { if (key in payload) patch[key] = dbText(payload[key]) || null; };
  uuid("colour_library_id"); uuid("shelf_colour_library_id");
  str("supplier_name"); str("shelf_supplier_name");
  ...
  if ("panel_options" in payload) {
    patch.panel_options = payload.panel_options && typeof payload.panel_options === "object" ? payload.panel_options : {};
  }
  ...
  // Per-SIDE end panel boards. Null means that end follows finish_panel_style,
  // which is what both ends did when they shared it.
  if ("end_left_style" in payload) patch.end_left_style = payload.end_left_style ?? null;
  if ("end_right_style" in payload) patch.end_right_style = payload.end_right_style ?? null;
  if ("banded_edges" in payload) patch.banded_edges = dbBandedEdges(payload.banded_edges);
  if ("hole_type" in payload) patch.hole_type = dbHoleType(payload.hole_type);
  if ("accessories" in payload) patch.accessories = dbAccessories(payload.accessories);
