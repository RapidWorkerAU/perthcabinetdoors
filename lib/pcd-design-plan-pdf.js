...
import { hasKickboard } from "./pcd-kickboard-utils";
import {
  PdfDocument,
  loadLogo,
  drawHeader,
...
const TYPE_LABELS = {
  ...
  corner_base_cabinet: "Corner base", corner_tall_cabinet: "Corner pantry", blind_corner_cabinet: "Blind corner",
  bookcase: "Bookcase", shelf_rail: "Shelf & rail",
...
// Height first, and the SCHEDULE_COLS heading says so. Change one and change
// the other: a size whose heading disagrees with it is worse than no heading.
...
function featuresText(item) {
  ...
  if (hasKickboard(item)) out.push("kickboard");
  if (item?.has_filler_panel) out.push("filler");
  if (item?.has_top_panel) out.push("top panel");
  if (item?.has_bottom_panel) out.push("underside");
...
const SCHEDULE_COLS = [
  ...
  { key: "size", label: "H x W x D", x: 162, w: 92 },
