 ...
 *
 * Pages are followed to the end of the window, up to `limit` per folder.
 */
export async function fetchMailboxMessages({ since = null, limit = 500, pageSize = 50 } = {}) {
  ...
  for (const folder of FOLDERS) {
    ...
    while (next && takenFromFolder < limit) {
      ...
      // not be able to turn one sync into an unbounded run.
      next = takenFromFolder < limit ? page["@odata.nextLink"] || null : null;
    }
  }

  // Oldest first, so a conversation is written in the order it happened and a
  // ticket's first message is genuinely its first.
  return collected.sort((a, b) => new Date(a.occurred_at || 0) - new Date(b.occurred_at || 0));
}
  ...
  for (const folder of FOLDERS) {
    ...
    while (next && taken < limit) {
      ...
      try {
        ...
        // Graph refuses some filters on some folders. One folder failing must
        // not cost the other.
        break;
      }
      for (const message of page.value || []) {
