...
// ── THE SAME WORDING, SPACED THE WAY IT WAS WRITTEN ──────────────────────────
//
// A paragraph with no style on it is spaced by whatever is reading it. In the
// editor that is our own rule, 8px under each one. In an email client it is the
// default, "margin: 1em 0", which is about 15px ABOVE and 15px below every
// paragraph, and mail clients do not all collapse the two the way a browser
// does. So one blank line typed between two sentences arrived as a gap four
// times the size of the one on screen, and the message read as though it had
// been written by somebody shouting across a room.
//
// INLINE, NOT A STYLE BLOCK. Gmail keeps a <style> in the head most of the
// time and Outlook does not, so anything that matters has to be on the tag
// itself. The sanitiser rebuilds every tag with no attributes at all, so there
// is nothing here to merge with and a plain replace is exact.
...
/**
 * Terms markup with the spacing an email client will actually honour.
 *
 * Give it what sanitizeTermsHtml produced. What comes back is the same words in
 * the same order, spaced like the editor rather than like a 1998 default.
 */
export function termsHtmlForEmail(html) {
  return String(html ?? "")
    // An empty paragraph is a deliberate blank line. With no content in it and
    // a margin of nearly nothing it would collapse to a gap you cannot see, so
    // it keeps a line break to stand it up. Written this way in every editor:
    // <p></p> from some, <p><br></p> from others.
    .replace(/<p>\s*<\/p>/gi, "<p><br></p>")
    .replace(/<(p|ul|ol|li)>/gi, (whole, tag) => {
      const lower = String(tag).toLowerCase();
      return `<${lower} style="${EMAIL_BLOCK_STYLES[lower]}">`;
    });
}
