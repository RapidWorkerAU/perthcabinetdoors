...
import { getDatabaseColourRows, isMadeToOrder, normaliseColourMaterialKey } from "./pcd-colour-library";
...
const MISS_MESSAGES = {
  ...
  // NOT a gap. Thermolaminate and compact laminate are made to order: the job
  // goes to the supplier and comes back priced for that job, so there is no
  // rate per m² to look up and there never will be. Said as its own answer so
  // nothing reports it as a missing price. See isMadeToOrder.
  made_to_order: "Made to order. This board is quoted by the supplier per job, so it is priced by hand rather than from a rate per m².",
...
// A row we matched but cannot take a rate from. Which of the two it is decides
// whether anyone should be told to go and fix something.
function asNoRate(row) {
  return isMadeToOrder(row)
    ? asMiss("made_to_order", { id: row.id, label: boardCostLabel(row), madeToOrder: true })
    : asMiss("unpriced", { id: row.id, label: boardCostLabel(row) });
}
...
/**
 * Whether a library row disagrees with the board written beside it.
 *
 * Only the four things that decide WHICH board it is, and only where the spec
 * actually says: a spec carrying nothing but an id (a public request, where the
 * row is all there ever was) has nothing to contradict and is trusted.
 *
 * Deliberately not the supplier. A supplier is often just the default that came
 * with a line, and disagreeing with it is not evidence the id is wrong; the
 * other four are.
 */
function rowContradictsSpec(row, spec = {}) {
  const wantMaterial = spec.material ? normaliseColourMaterialKey(spec.material) : "";
  if (wantMaterial && normaliseColourMaterialKey(row.material_type) !== wantMaterial) return true;

  const wantThickness = thicknessMm(spec.thickness);
  if (wantThickness && thicknessMm(row.thickness) !== wantThickness) return true;

  const wantColour = lower(colourWithoutFinishPrefix(spec.colour, spec.finish));
  if (wantColour && lower(row.name) !== wantColour) return true;

  const wantFinish = lower(spec.finish);
  if (wantFinish && lower(row.finish_type) && lower(row.finish_type) !== wantFinish) return true;
...
export function matchBoardCost(rows = [], spec = {}) {
  ...
  if (colourLibraryId) {
    ...
    // BUT ONLY IF IT IS STILL THE SAME BOARD. This trusted the id outright, and
    // that put a wardrobe on a quote reading "Black Texture 16mm" and priced
    // from a Laminex Black AbsoluteMatte 18mm at $145.49 a square metre. The id
    // had been left behind by a screen that rewrote the words on the item and
    // not the row it pointed at, and nothing downstream ever looked at the two
    // together. An id that contradicts the board beside it is not the board
    // somebody clicked, it is a leftover, so it falls through to name matching
    // below, which refuses to guess between two prices and says so.
    if (exact && !rowContradictsSpec(exact, spec)) {
      const match = asMatch(exact, "id");
      return match.costPerSqmExGst > 0 ? match : asNoRate(exact);
    }
  ...
  if (wantSupplier) {
    ...
    if (bySupplier.length) {
      ...
      if (!priced.length) return asNoRate(bySupplier[0]);
  ...
  if (!priced.length) return asNoRate(candidates[0]);
  if (distinctPrices(priced).size > 1) return asMiss("ambiguous");
...
export function boardCostLinePatch(match, { areaSqm = 0 } = {}) {
  ...
  const calculated = areaSqm > 0 && rate > 0 ? roundMoney(areaSqm * rate) : 0;
