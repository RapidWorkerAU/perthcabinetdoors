...
/**
 * WHERE THE HINGES GO, IN THE WORDS THEY WERE ASKED IN.
 *
 * Every screen that captures a position asks it the same way: the bottom hinge
 * up from the bottom edge, the top hinge down from the top edge, and any hinge
 * between them up from the bottom. So this reads it back the same way.
 *
 * It used to read everything back from the bottom, so a customer who typed
 * "100 from the top" on a 700 door was shown "600" and reasonably thought we
 * had made a mistake. Same numbers they typed, same edges they measured from.
 *
 * Null when the pattern was left to us, so a caller can say "our standard".
 */
const ORDINALS = ["2nd", "3rd", "4th", "5th"];
export function hingePositionLines(line = {}) {
  const read = readHinges(line);
  if (!read.drills || read.fromBottom === null || read.fromTop === null) return null;
  const middles = read.middles.length
    ? read.middles
    : evenMiddles({ height: read.height, count: read.count, fromBottom: read.fromBottom, fromTop: read.fromTop });
  return [
    `Bottom hinge ${read.fromBottom}mm from bottom`,
    ...middles.map((mm, i) => `${ORDINALS[i] || `${i + 2}th`} hinge ${mm}mm from bottom`),
    `Top hinge ${read.fromTop}mm from top`,
  ];
}
...
export function hingeSummaryLines(line = {}) {
  ...
  // Which boring. Two machine setups, and a door bored for one will not take a
  // hinge made for the other, so it sits with the drilling rather than in notes.
  const boring = holeTypeOf(line);
  if (boring) out.push(`Hinge holes: ${boring}`);

  // Read back the way they were asked. See hingePositionLines.
  out.push(...(hingePositionLines(line) || ["Hinge positions: standard PCD positions"]));
  return out;
}

/**
 * THE DRILLING AS THE CUSTOMER IS TOLD IT, on the quote viewer and on the quote
 * PDF, from here so the two can never come to say different things.
 *
 * NOT DRILLING IS AN ANSWER. hingeSummaryLines says nothing at all for a line
 * that is not drilled, which is right where it is added to a list of details.
 * A customer reading a quote is checking every door, and silence on one of them
 * reads as an omission rather than as a decision, so this always says something.
 *
 * Positions are read back the way they were asked (see hingePositionLines),
 * and a line that left the pattern to us says so rather than showing nothing.
 */
export function hingeCustomerLines(line = {}) {
  const read = readHinges(line);
  if (!read.drills) return ["No hinge holes"];

  const out = [read.count ? `Drilled, ${read.count} ${read.count === 1 ? "hinge" : "hinges"}` : "Drilled"];
  if (read.side) out.push(`Hinged ${read.side.toLowerCase()}`);
  const boring = holeTypeOf(line);
  if (boring) out.push(`${boring} holes`);

  out.push(...(hingePositionLines(line) || ["Standard PCD positions"]));
  return out;
}
 ...
 */
/** Which boring the line asked for, read off either shape of line. */
export function holeTypeOf(line = {}) {
  return text(line.hole_type ?? line.holeType);
}
...
export function hingeProblems(line = {}) {
  ...
  // Blum Inserta or a 35mm cup, and nothing else. There used to be a third
  // answer, "your standard", which told the customer nothing and the workshop
  // less. A door bored for one will not take a hinge made for the other, so an
  // unanswered one is something we need before it is made, like the side.
  if (!holeTypeOf(line)) problems.push("which kind of hinge hole");
