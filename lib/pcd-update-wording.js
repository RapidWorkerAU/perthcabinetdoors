...
export const ITEM_STATUS_WORDS = {
  ...
  //
  // TBC IS SAID OUT LOUD, and only while it is true. Ordered with no date behind
  // it is the point at which a customer starts wondering, so the line answers
  // the question before it is asked. The moment an ETA is entered the TBC stops
  // appearing and the expected arrival sentence takes over instead, so the two
  // never contradict each other in the same email.
  Ordered: (n, on, supplier, etaKnown = true) =>
    `${n} ordered${supplier}${on ? ` on ${longDate(on)}` : ""}${etaKnown ? "" : " - Item completion date TBC."}`,
...
/**
 * A booking, named the way the customer would name it.
 *
 * Only the three somebody has to be home for. A reminder is ours, and "other"
 * has no description anybody wrote with a customer in mind.
 */
export const BOOKING_WORDS = {
  measure: "Your site measure",
...
export function sentenceFor(change, options = {}) {
  ...
  switch (change.kind) {
    ...
    case "item_status": {
      ...
      return word(noun, change.on, supplier, change.etaKnown !== false);
    }

    case "item_ordered":
      // The date on its own, for the rare case somebody records when a batch
      // was ordered without moving its status at the same time.
      return `${change.qty} x ${change.itemLabel} ordered on ${longDate(change.to)}` +
        (change.etaKnown === false ? " - Item completion date TBC." : "");
    ...
    // ── bookings ──────────────────────────────────────────────────────────
    //
    // The one thing on this list somebody has to be home for, so it is the one
    // that most needs saying and the one most worth getting the tone right on.
    // A booked date is a real appointment we control, so unlike a supplier ETA
    // it is stated plainly rather than hedged.
    case "booking_made":
      return `${BOOKING_WORDS[change.bookingKind] || "Your booking"} is booked for ${longDate(change.to)}.`;

    case "booking_moved":
      // Says it moved rather than quietly restating a new date. Somebody may
      // have taken the day off for the old one.
      return (
        `${BOOKING_WORDS[change.bookingKind] || "Your booking"} has moved from ` +
        `${shortDate(change.from)} to ${longDate(change.to)}.`
      );

    case "booking_cancelled":
      // No reason given: whatever was typed on the booking was written for us.
      // "We will be in touch" is the promise that makes this survivable.
      return `${BOOKING_WORDS[change.bookingKind] || "Your booking"} has been cancelled. We will be in touch to rebook.`;

    case "variation_approved":
      // They pressed the button; this confirms it reached us. Applied is a
      // separate line and says the order actually changed.
      return `Thanks for approving change ${change.reference}.`;
    ...
    case "variation_applied":
      // A change we absorbed. No figure at all, because there is nothing for
      // them to tie it back to and a number beside "no extra cost" reads as a
      // price somebody might still be asked for.
      if (change.held) {
        return `Change ${change.reference} has been made to your order at no extra cost.`;
      }
...
export function internalLabelFor(change) {
  switch (change.kind) {
    ...
    case "item_ordered":    return "Ordered date set";
    ...
    case "booking_moved":     return "Booking moved";
    case "booking_cancelled": return "Booking cancelled";
    case "variation_approved":return "Variation approved";
    case "order_complete":   return "Order marked complete";
    case "variation_applied":return change.held ? "Variation applied at no charge" : "Variation applied";
...
/**
 * The two lines at the top, above the dated updates.
 *
 * BOTH COME FIRST, and that is the change: the notice used to sit at the bottom
 * as two paragraphs of hedging that read like a disclaimer nobody asked for.
 * Said once, at the top, it sets the expectation before the updates rather than
 * apologising for them afterwards.
 *
 * "No reply is required unless" is doing the work the old reply invitation did,
 * without asking for a reply we do not need.
 */
export const UPDATE_OPENING = (orders = 1) =>
  `We have made progress on your ${orders > 1 ? "orders" : "order"} this week. See details below.`;

export const UPDATE_AUTOMATIC_NOTE =
  "This is an automated update from the Perth Cabinet Doors order management system. No reply is required " +
  "unless you would like to clarify something about your order.";

/** Signed off like a person wrote it, because a person pressed send. */
export const UPDATE_SIGN_OFF = ["Kind Regards,", "Perth Cabinet Doors"];
...
export function updateEmailBody({ customerName, orders = [] }, options = {}) {
  ...
  lines.push(`Hi ${first},`);
  lines.push("");
  lines.push(UPDATE_OPENING(sections.length));
  lines.push("");
  ...
  lines.push("");
  lines.push(...UPDATE_SIGN_OFF);
  return lines.join("\n");
}

export function updateEmailSubject() {
  return "Order Update - Perth Cabinet Doors";
}
