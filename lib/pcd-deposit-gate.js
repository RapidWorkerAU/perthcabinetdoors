...
// ── THE WEB SHOP GOES THROUGH THIS GATE TOO ──────────────────────────────────
//
// A web order is a quote the shop's checkout wrote and nobody sent, paid in
// full rather than by deposit. Everything that makes this gate safe to reach
// three ways at once, the conditional claim, the one order per quote, the
// sweep, is exactly what a paid cart needs, so it comes through here rather
// than a second path that would have to be kept in step with this one.
//
// It holds its own status while it waits, not awaiting_deposit, so an unpaid
// cart never lands on the quotes list, the deposit chase, the dashboard or the
// expiry clock. When its payment page runs out it is archived quietly, with no
// email to anybody. Decided 11 September 2026.
export const WEB_CHECKOUT = "web_checkout";

/** The flow name a web order's Stripe session carries. */
export const WEB_ORDER_FLOW = "web_order";

/** Every flow whose money becomes an order here. */
export const GATE_FLOWS = new Set(["quote_deposit_gate", WEB_ORDER_FLOW]);

/** The statuses a quote can be claimed from when its money arrives. */
const HELD_STATUSES = [AWAITING_DEPOSIT, WEB_CHECKOUT];
...
export async function finaliseDepositAcceptance(supabase, session, { request = null } = {}) {
  ...
  // Paid in full on the website, rather than a deposit on a quote we sent.
  const webOrder = session?.metadata?.flow === WEB_ORDER_FLOW || quote.status === WEB_CHECKOUT;
  ...
  const { data: payment, error: paymentError } = await supabase
    ...
    .insert({
      order_id: orderId,
      // A web order is paid in full up front, so it is the final payment and
      // the order owes nothing from the moment it exists.
      payment_type: webOrder ? "final" : "deposit",
      amount,
      is_paid: true,
      paid_at: paidOn,
      request_status: "paid",
      sort_order: 0,
      notes: webOrder
        ? `Paid in full online for web order ${quote.quote_number}`
        : `${Number(quote.deposit_percent || 0).toFixed(2)}% deposit paid to accept ${quote.quote_number}`,
      stripe_checkout_session_id: session.id,
      stripe_payment_intent_id: session.payment_intent || null,
  ...
  const actionRow = {
    ...
    note: webOrder ? "Ordered and paid in full on the website." : "Approved and deposit paid.",
  };
  try {
  ...
  return { ok: true, orderId, payment, quote, amount, webOrder, alreadyDone: false };
}
...
export async function markCheckoutExpired(supabase, sessionId) {
  ...
  // A web order has nobody to come back to it: the cart that made it can make
  // another. So the quote behind an unpaid one is archived here, quietly, and
  // only while it is still waiting. A deposit quote is untouched.
  if (quoteId) await archiveWebCheckout(supabase, quoteId);
  return quoteId;
}

/**
 * Put away the quote behind a web checkout that was never paid.
 *
 * Only a quote still waiting on its payment: the conditional write means one
 * that was paid in the same second is never archived out from under its order.
 * Never throws. Housekeeping must not fail the webhook or the sweep.
 */
export async function archiveWebCheckout(supabase, quoteId) {
  try {
    await supabase
      .from("pcd_quotes")
      .update(quoteArchivePatch({ status: WEB_CHECKOUT }, ARCHIVED_EXPIRED))
      .eq("id", quoteId)
      .eq("status", WEB_CHECKOUT);
  } catch (error) {
    console.error(`[deposit-gate] could not archive web checkout ${quoteId}: ${error?.message || error}`);
  }
}

/**
 * The customer came back from Stripe without paying and is trying again.
 *
 * The page they left is closed at Stripe before a new one is opened, so there
 * are never two live ways to pay for one cart, and the quote behind it is put
 * away. Never throws.
 */
export async function abandonWebCheckout(supabase, quoteId) {
  if (!quoteId) return;
  const { data: quote } = await supabase.from("pcd_quotes").select("id, status").eq("id", quoteId).maybeSingle();
  if (quote?.status !== WEB_CHECKOUT) return;
  await cancelOpenCheckouts(supabase, quoteId, { status: "superseded" });
  await archiveWebCheckout(supabase, quoteId);
}
