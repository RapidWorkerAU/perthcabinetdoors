...
function requestLineFrom(line, item) {
  ...
  return {
    ...
    // The kind of panel, the banded edges (all four on decorative board unless
    // changed) and the hinge boring, from the same shared translation the admin
    // import uses. The public planner asks none of them; these are the defaults
    // the design carries, for us to confirm on the quote.
    panelUse: line.panel_use || "",
    ...(Array.isArray(line.banded_edges) ? { bandedEdges: line.banded_edges } : {}),
    holeType: line.hole_type || "",
