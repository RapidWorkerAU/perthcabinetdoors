...
import { normaliseHingeSide } from "./pcd-hinges";
...
function doorLine(item, size, board, trace, sectionLabel) {
  ...
  const drills = size.hingeQty > 0;

  // THE PLANNER ALREADY KNOWS ALL OF THIS. It works out the handing and the
  // exact cup positions to draw the elevation, and it used to hand them over as
  // a sentence in the notes, which somebody then had to read and retype into a
  // form. That retyping is where a pair becomes two identical doors.
  //
  // Positions are stored bottom-to-top as distances from the bottom, which is
  // the same datum the elevation drills them from. The form asks for the top
  // cup as a distance from the TOP, so it is turned round here once rather
  // than everywhere that reads it.
  const positions = drills && Array.isArray(size.hingePositions)
    ? size.hingePositions.map((mm) => Math.round(Number(mm) || 0)).filter((mm) => mm > 0)
    : [];
  const height = Number(size.height) || 0;
  const hasPositions = positions.length >= 2 && height > 0;
  ...
  return {
    ...
    ...board,
    hingeHoles: drills,
    hingeQty: drills ? `${size.hingeQty} hinges` : "",
    // A corner leaf has no handing of its own: a bi-fold's is fixed by which
    // leaf is frame-hinged, so the planner omits it and so does this.
    hingeSide: drills ? normaliseHingeSide(size.hingeSide === "L" ? "Left" : size.hingeSide === "R" ? "Right" : "") : "",
    hingeFromBottomMm: hasPositions ? positions[0] : null,
    hingeFromTopMm: hasPositions ? Math.max(0, height - positions[positions.length - 1]) : null,
    hingeMiddlesMm: hasPositions ? positions.slice(1, -1) : [],
    // The sentence is gone. It said the same thing the fields now say, and two
    // copies of a measurement is one copy that can be wrong.
    notes: join(`Door for ${where}.`, colourNote(board)),
  };
}
...
// One end panel's board. Each end can be its own — an exposed end on show in a
// profiled board beside a plain end that dies into a run is an ordinary job —
// so `overrideKey` names that side's style. Falling through to the shared
// finishing panel is what both ends did before they could differ, so a design
// that never set a side keeps exactly the board it had.
function endPanelBoard(item, overrideKey) {
  const owned = isCustomerOwned(item);
  const side = item[overrideKey];
  const hasOwn = Boolean(side && (side.material || side.colour));
  const style = hasOwn ? side : (item.finish_panel_style || item.door_style || {});
  return boardFields(style, owned ? null : item, PANEL_THICKNESS_MM);
}
...
function endPanelLines(item, roomName, roomHeightMm) {
  ...
  // Each end is priced off its OWN board, so two ends in different boards reach
  // the quote as two different lines rather than one shared spec.
  if (item.end_panel_left) {
    lines.push(panelLine(item, "End panel (left)", sideDepth, height, endPanelBoard(item, "end_left_style"), trace, join("Finished side, depth × height.", reach)));
  }
  if (item.end_panel_right) {
    lines.push(panelLine(item, "End panel (right)", sideDepth, height, endPanelBoard(item, "end_right_style"), trace, join("Finished side, depth × height.", reach)));
  }
