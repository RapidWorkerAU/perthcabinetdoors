...
function boardFields(style, fallback) {
  const s = style || {};
  ...
  return {
    ...
    thickness: s.thickness_mm ? `${s.thickness_mm}mm` : "",
...
function doorLines(item, roomName) {
  ...
  const board = boardFields(item.door_style, owned ? null : item);
...
function drawerLines(item, roomName) {
  const trace = traceLabel(item, roomName);
  const owned = isCustomerOwned(item);
  const board = boardFields(item.drawer_style, owned ? null : item);
...
function panelBoard(item) {
  ...
  return boardFields(style, owned ? null : item);
}
...
function bodyLines(item, roomName) {
  ...
  const board = boardFields(item.door_style, item);
