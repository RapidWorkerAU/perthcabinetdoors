...
/**
 * Drilling for one order line, read through to the quote line it came from.
...
export function drillingForItem(item, quoteLinesById) {
  ...
  if (!quoteLine) return { drill: LABEL_UNKNOWN, hinges: "" };
  if (!quoteLine.hinge_holes) return { drill: "No", hinges: "" };
  return { drill: "Yes", hinges: text(quoteLine.hinge_qty) };
}
...
function sizeOf(spec) {
  ...
  return `${width ? `${width}mm` : "-"} x ${height ? `${height}mm` : "-"}`;
}
    ...
    .flatMap((row) => {
      ...
      return Array.from({ length: total }, (_, index) => index).flatMap((index) =>
        variants.map((variant) => {
          ...
          const block = materialBlock(parts, row.thickness, library);
