 ...
 * Returns { drill, hinges, holeType }, where drill is "Yes", "No" or LABEL_UNKNOWN.
 *
...
export function drillingForItem(item, quoteLinesById) {
  ...
  if (drills === null || drills === undefined) return { drill: LABEL_UNKNOWN, hinges: "", holeType: "" };
  if (!drills) return { drill: "No", hinges: "", holeType: "" };
  return {
    drill: "Yes",
    hinges: text(item?.hinge_qty || quoteLine?.hinge_qty),
    holeType: text(item?.hole_type || quoteLine?.hole_type),
  };
}

/**
 * The boring, short enough for the label's drilling box. The box is the width
 * of the order number above it, and "Blum Inserta" set small enough to fit is
 * harder to read at the drill than the one word that tells the two apart.
 */
export function holeTypeShort(holeType) {
  const value = text(holeType).toLowerCase();
  if (!value) return "";
  if (value.includes("inserta")) return "INSERTA";
  if (value.includes("35")) return "35MM CUP";
  return text(holeType).toUpperCase();
}
...
export function buildCutListLabels({
  ...
  const fromRows = (rows, section, options = {}) => rows
    ...
    .flatMap((row) => {
      ...
      const { drill, hinges, holeType } = drillingForItem(item, quoteLinesById);
      ...
      return Array.from({ length: total }, (_, index) => index).flatMap((index) =>
        variants.map((variant) => {
          ...
          // Which boring, under the count. Blank when not drilled or not known,
          // and then the label keeps its old height.
          holeType: drill === "Yes" ? holeTypeShort(holeType) : "",
