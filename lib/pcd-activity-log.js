...
const FRIENDLY_ACTIVITY_LABELS = {
  ...
  painting_cost_ex_gst: "Painting cost ex GST",
  glass_cost_ex_gst: "Glass cost ex GST",
  removal_cost_ex_gst: "Door removal and disposal cost ex GST",
...
export async function logOrderActivity(supabase, activity) {
  ...
  try {
    // Which customer this happened to, so quotes sent, payments taken and
    // variations appear on their desk beside the emails about them. Worked out
    // from the quote when the caller has not said, because most callers know
    // the quote and not the customer.
    let customerId = activity.customer_id || null;
    if (!customerId && activity.quote_id) {
    ...
    if (!customerId && activity.order_id) {
      const { data } = await supabase
        .from("pcd_orders")
        .select("customer_id")
        .eq("id", activity.order_id)
        .maybeSingle();
      customerId = data?.customer_id || null;
    }
    if (!customerId && activity.quote_request_id) {
      const { data } = await supabase
        .from("pcd_quote_requests")
        .select("customer_id")
        .eq("id", activity.quote_request_id)
        .maybeSingle();
      customerId = data?.customer_id || null;
    }
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        variation_id: activity.variation_id || null,
