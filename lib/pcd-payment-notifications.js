...
export function paymentTypeLabel(paymentType, flow) {
  ...
  // Bought and paid for on the website. Said as what it is, because sales@ has
  // never had a web order before and "Final Payment" on a job nobody quoted
  // reads as a mistake.
  if (flow === "web_order") return "Web Order Payment";
  const labels = {
...
function nextStepsForPayment({ payment, flow }) {
  if (flow === "web_order") {
    return "A customer has bought and paid in full on the website. The order is live and on the board. Open it in PCD Admin to plan the cut and the delivery.";
  }
