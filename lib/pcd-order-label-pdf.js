...
export const LABEL_LAYOUT_VERSION = "labels-2026-09-11-hole-type-v11";
...
// Which boring, under the count. Blum Inserta and a bare 35mm cup are two
// machine setups and the wrong one is a remade door, so the piece says which.
// Only on a label that has one, so every other label keeps its old height.
const DRILL_TYPE_HEIGHT = DRILL_QTY_HEIGHT;
...
/**
 * HEIGHT IS FIRST IN THE STRING, because it is first everywhere in this
 * business. sizeOf in pcd-order-labels.js and itemSizeLabel in
 * pcd-order-item-label.js both write `${height} x ${width}`.
 *
 * This read it the other way round, so the two numbers were printed in the
 * right order with the WRONG LETTERS BESIDE THEM: the top box said W against
 * the height, the bottom said H against the width. Nothing looked broken, and a
 * 600 x 508 door and a 508 x 600 door are both plausible, so the only way to
 * catch it was to know which number should be on top.
 */
...
/** Top box then bottom box. Height above width, matching the numbers. */
...
function drillTableHeight(label) {
  return DRILL_CAPTION_HEIGHT + DRILL_VALUE_HEIGHT + DRILL_QTY_HEIGHT + (clean(label?.holeType) ? DRILL_TYPE_HEIGHT : 0);
}
...
function instructionHeight(measFont, label) {
  return Math.max(drillTableHeight(label), dimensionsInkHeight(measFont));
}
...
function labelContentHeight(label, logo, measFont) {
  const bands = [
    mastheadHeight(logo),
    instructionHeight(measFont, label),
...
function drawInstructions(page, label, top, measFont) {
  const left = LABEL_MARGIN;
  const right = LABEL_WIDTH - LABEL_MARGIN;
  const height = instructionHeight(measFont, label);
  ...
  const y = top + Math.max(0, (height - drillTableHeight(label)) / 2);
  ...
  // Which boring, captioned the same way as the count above it. The caption
  // is the same width as QTY's so the two values line up.
  const holeType = clean(label.holeType);
  if (holeType) {
    const typeY = qtyY + DRILL_QTY_HEIGHT;
    const typeCaption = Math.max(qtyCaption, boldTextWidth("TYPE", DRILL_CAPTION_FONT) + CELL_PAD * 2);
    fillCell(page, {
      x: tableX, y: typeY, width: typeCaption, height: DRILL_TYPE_HEIGHT,
      text: "TYPE", size: DRILL_CAPTION_FONT,
    });
    outlineCell(page, {
      x: tableX + typeCaption, y: typeY, width: DRILL_COLUMN_WIDTH - typeCaption, height: DRILL_TYPE_HEIGHT,
      text: holeType, bold: true,
      size: fitFont([holeType], DRILL_COLUMN_WIDTH - typeCaption - CELL_PAD * 2, DRILL_VALUE_FONT, 6, true),
    });
  }
