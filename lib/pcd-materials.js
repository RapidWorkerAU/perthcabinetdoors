...
export const MATERIALS = [
  ...
  // LAMINATE IS SOLD AS A SHEET. IT IS NOT WHAT ANYTHING ELSE IS MADE FROM.
  // A door, a drawer front and a table top are all cut from a board; laminate
  // is the 0.7mm sheet itself and it is bought as the sheet. That is what
  // "standalone" says, and materialsForProductType below keeps it out of every
  // type's material list because of it. Without that flag it would show up as a
  // material option under Door, and somebody would eventually pick it.
  {
    key: "laminate",
    label: "Laminate",
    design: "laminate",
...
// "Laminate" is a staff type. It is on this list so the quote editor, the
// variation editor and the Excel order form all offer it, and PRODUCT_FIELDS
// marks it internal so productTypeChoices drops it and the public request form
// never does. That is deliberate: it sits next to decorative board and is
// cheaper per sheet, so a customer picking it when they wanted a door is a
// correction we would be making forever.
export const PRODUCT_TYPES = ["Door", "Drawer front", "Panel", "Table top", "Hardware", "Laminate"];
...
export const MATERIALS_ALLOWED_BY_TYPE = {
  ...
  // The sheet IS the product, so the only material it can be is itself. Named
  // rather than left unrestricted, so the order form's material column for a
  // Laminate row offers the one right answer instead of every board we sell.
  Laminate: ["Laminate"],
};
...
export function materialsForProductType(productType) {
  ...
  // An unrestricted type gets every material a thing can be MADE FROM, which is
  // not the same as every material we sell. A standalone sheet is sold as
  // itself and is never the stock another product is cut out of, so it is only
  // ever offered to a type that names it above.
  if (!allowed) return MATERIALS.filter((m) => !m.standalone).map((m) => m.label);
...
export function normaliseMaterialKey(material) {
  ...
  // LAST, ALWAYS. "thermolaminate" and "compact laminate" both contain
  // "laminate", so this line has to sit below both of them or it swallows them
  // and every thermolaminate colour in the library resolves to the wrong
  // material.
