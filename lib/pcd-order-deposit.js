...
//
// `currentAcceptedAt` is what the order already holds, and it is what stops
// accepted_at being a moving target. See the note on the stamp below.
export function depositUpdates(depositRows, currentStatus, currentAcceptedAt = null) {
  ...
  if (paid) {
    ...
    //
    // ONCE, AND NEVER AGAIN. This used to be written on every run, and this
    // function runs on every deposit sync: the Stripe webhook firing twice, a
    // second payment recorded against the job, somebody unticking and reticking
    // a deposit. Each of those moved accepted_at to today, so "when the order
    // was raised" quietly became "the last time anybody touched the money", and
    // every age measured from it on the work board reset with it.
    //
    // An order that already carries the date keeps it. There is no case where
    // the moment a job was first accepted should change afterwards.
    if (!currentAcceptedAt) updates.accepted_at = new Date().toISOString();
...
export async function syncDepositFields(supabase, orderId) {
  ...
  // The current status decides whether promotion is allowed, and the accepted
  // date decides whether there is anything left to stamp, so both have to be
  ...
  const updates = depositUpdates(deposits, order?.status, order?.accepted_at);
