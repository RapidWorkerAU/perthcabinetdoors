...
export function requestCards(rows, today) {
  return (rows || []).map((row) =>
    card({
      ...
      why: "Quote request never converted to a sent quote, and no reply sent to that address.",
      days: daysSince(row.created_at, today),
...
export function chaseCards({ quotes, payments, variations }, today) {
  ...
  const fromQuotes = (quotes || []).map((row) => {
    const theyReplied = Boolean(row.repliedAt && row.sent_at && row.repliedAt > row.sent_at);
    return card({
      id: `quote:${row.id}`,
