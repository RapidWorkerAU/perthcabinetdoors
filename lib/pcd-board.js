...
function card(input) {
  ...
  return {
    ...
    days,
    late: days >= LATE_AT,
    // THE NEWEST THING, WHERE THAT IS NOT THE THING BEING TIMED.
    //
    // A card is timed from the OLDEST unanswered conversation, because that is
    // the one actually overdue an answer and the one worth sorting by. But a
    // person with several conversations waiting can have written again this
    // morning, and a card reading "45d since they wrote" on its own says we
    // have missed somebody when we have been talking to them all week.
    //
    // So where the newest is not the one being timed, it is carried too and
    // shown beside it. Null when they are the same, which is every card with
    // one conversation on it, so nothing gains a second number for no reason.
    latest: Number.isFinite(input.latest) && input.latest < days ? Number(input.latest) : null,
...
export function replyCards({ enquiries, tickets }, today) {
  const fromEnquiries = (enquiries || []).map((row) =>
    card({
      ...
      // Their desk if we know who they are, because that is where a reply is
      // typed. A sender with no record yet has no desk, so it opens their row
      // in the enquiries list rather than the top of it. See useFocusedRow.
      href: row.customerId ? `/admin/customers/${row.customerId}` : `/admin/enquiries?focus=${row.id}`,
    })
  );
  ...
  const fromTickets = (tickets || []).map((row) => {
    ...
    return card({
      ...
      // And their newest, so a card carrying an old conversation cannot read as
      // a customer we have ignored for a month when they wrote again yesterday.
      latest: daysSince(row.newestInbound || row.last_message_at, today),
      tags: [
...
export function requestCards(rows, today) {
  return (rows || []).map((row) =>
    card({
      ...
      // The draft quote when there is one, because that is where the work is
      // carried on. Otherwise the request's own row rather than the list it
      // sits in: a card that names one person and then lands on a page of two
      // hundred makes you find it again by eye. See useFocusedRow.
      href: row.draftQuoteId ? `/admin/quotes/${row.draftQuoteId}` : `/admin/quote-requests?focus=${row.id}`,
    })
  );
}
...
export function materialCards(rows, today) {
  return (rows || []).map((row) => {
    ...
    return card({
      ...
      who: row.customer_name || row.order_number || "Order",
      what: row.name || "Order",
      // "NOT STARTED" COVERS BOTH LISTS, AND IT HAS TO.
      //
      // This said "still Not Ordered", which is the first word on the supplier
      // list and means nothing on a panel we cut ourselves: ours sit at Not
      // Started until somebody touches them, and their order status column
      // stays on its default forever. So the count now spans both lists and the
      // words have to as well, or the card names a state half the panels it is
      // counting were never in.
      why: `${row.notOrdered} panel${row.notOrdered === 1 ? "" : "s"} not started, and the job is booked to start ${
      ...
      subjectType: "order",
      stamp: row.updated_at || row.accepted_at || null,
      tags: [[row.order_number, "ref"], [`${row.notOrdered} not started`, "miss"]],
