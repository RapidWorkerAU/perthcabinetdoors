...
export function profileLibraryRowFromDraft(draft = {}) {
  ...
  return {
    ...
    // Normalised BEFORE the check, or a row arriving as "laminex" fails to
    // match and is silently rewritten as a Polytec one.
    supplier_name: PROFILE_LIBRARY_SUPPLIERS.includes(normaliseSupplierName(draft.supplier_name))
