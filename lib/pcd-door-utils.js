...
import { endPanelThicknesses } from "./pcd-finishpanel-utils";
...
export function mixedBaySections(item) {
  ...
  const cabinetH = Number(item?.height_mm) || total || 0;
...
export function finishedSidePanelDepthMm(item) {
  ...
  if (!frontHasAppliedPanel(item) || frontPanelMode(item) !== FRONT_PANEL_MODE_INSET) return depth;
  return depth + frontPanelThicknessMm(item);
}
...
export function computeDoorSizes(item) {
  return computeDoorSizesForConfig(item.door_config || {}, frontSizingWidthMm(item), item.height_mm);
}
...
export function computeDrawerSizes(item) {
  return computeDrawerSizesForConfig(item.drawer_config || {}, frontSizingWidthMm(item), item.height_mm);
}
