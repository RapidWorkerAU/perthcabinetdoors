...
/**
 * WHERE A CABINET'S OWN SHELVES SIT, measured up from the cabinet bottom.
 *
 * The same rule a bay's shelves follow above: the saved positions win, but only
 * while there are as many of them as the shelf count says. The views used to
 * draw every saved position whatever the count, so a cabinet whose shelves had
 * been dragged or imported at two, then set to one, went on showing two.
 *
 * When the count and the saved positions disagree the shelves are spread
 * evenly, (i+1) x H / (qty+1), which is where an un-dragged shelf has always
 * sat, so the cut list schematic and the views agree about it.
 */
export function cabinetShelfHeightsMm(item) {
  const qty = Math.max(0, Math.round(Number(item?.shelf_qty) || 0));
  if (!qty) return [];
  const height = Number(item?.height_mm) || 720;
  const stored = Array.isArray(item?.shelf_heights_mm) ? item.shelf_heights_mm.map((h) => Number(h) || 0) : [];
  if (stored.length === qty && stored.every((h) => h > 0)) return stored.map((h) => Math.min(h, height));
  return Array.from({ length: qty }, (_, i) => Math.round(((i + 1) * height) / (qty + 1)));
}
