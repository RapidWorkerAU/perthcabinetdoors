...
export function computeBenchtopRun(item, allItems) {
  ...
  // Flush end-panel cover (single cabinet): the top extends past the carcass by
  // a finished end panel's thickness at each exposed end, so it sits OVER the
  // panel rather than beside it — except a waterfall end, where the top drops to
  // the floor instead. Uses the same low/high span the plan reserves for the
  // panel, so the drawing, 3D and quote all agree. Multi-cabinet runs keep the
  // plain carcass-run width for now (far-end panel cover not yet applied).
  const single = (length) => {
