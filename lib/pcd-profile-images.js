...
export function profileImageFallbackSrc(family, name) {
  if (!PROFILE_IMAGE_FAMILIES.includes(family) || !name) return null;
  // \s, not s. Without the backslash this matches the LETTER s, so the fallback
  // for "Classic Square" asked for "cla--ic--quare.jpg" and the tile that was
  // meant to rescue a failed load failed too.
  const slug = String(name).trim().toLowerCase().replace(/\s+/g, "-");
