...
export function buildProposedRows(context) {
  return (context?.proposedAdditions || []).map(({ line, variation }) => ({
    ...
    qty: line.qty || 1,
    size: line.width_mm || line.height_mm ? cutSize(line.width_mm, line.height_mm) : "-",
...
export function buildMadeToOrderRows(items, context) {
...
function quoteNoteContainerHeight(value, width) {
  const lineCount = Math.min(
    wrappedTextLines(value, quoteNoteBodyWidth(width), QUOTE_NOTE_BODY_SIZE).length,
    QUOTE_NOTES_MAX_LINES
  );
...
function drawQuoteNoteContainer(page, title, value, y, width) {
  const height = quoteNoteContainerHeight(value, width);
  page.fillColor(PANEL);
  page.strokeColor(LINE);
  page.lineWidth(0.7);
  ...
  drawWrappedText(
    page,
    value,
    MARGIN + QUOTE_NOTE_PAD_X,
    y + QUOTE_NOTE_HEADER_H + 6,
    quoteNoteBodyWidth(width),
    QUOTE_NOTE_BODY_SIZE,
...
function quoteSummaryBlocks(quote, totals, contentWidth) {
  ...
  quoteNotesSections(quote).forEach(([title, value]) => {
    blocks.push({
      height: quoteNoteContainerHeight(value, contentWidth),
      gap: QUOTE_NOTE_GAP,
      draw: (page, y) => drawQuoteNoteContainer(page, title, value, y, contentWidth),
    });
  });
  return blocks;
}
...
const MTO_COLUMN_SHARES = [
  ...
  { key: "index", label: "#", share: 3 },
  { key: "item", label: "Item", share: 18, flag: true },
  { key: "qty", label: "Qty", share: 3 },
  { key: "size", label: "Size", share: 12 },
  { key: "material", label: "Material / colour", share: 20 },
  { key: "supplier", label: "Supplier", share: 10 },
  { key: "ref", label: "Ref", share: 9 },
  { key: "ordered", label: "Ordered", share: 7 },
  { key: "eta", label: "ETA", share: 7 },
  { key: "status", label: "Status", share: 7 },
];
...
function drawNotesPage(page, order, pageNumber, pageCount) {
 ...
 */
export function generateOrderCutListPdf({ order, items, quoteLines = [], variations = [], variationLines = [], panelNumbers = null }) {
  const context = buildVariationContext({ variations, variationLines });
  const rawCutRows = buildCutListRows(items, context);
  const rawMtoRows = buildMadeToOrderRows(items, context);
  ...
  const logo = loadLogo();
  const pdf = new PdfDocument({ logo });
  const pageCount = cutPages.length + mtoPages.length + 1;
  let pageNumber = 0;
  ...
  pdf.addPage((page) => {
    ...
    page.logoWidth = logo?.width || 1;
    page.logoHeight = logo?.height || 1;
    drawNotesPage(page, order, pageCount, pageCount);
  });
