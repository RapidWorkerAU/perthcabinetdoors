...
import { hingeSummaryLines } from "./pcd-hinges.js";
...
function quoteLineDetails(line) {
  ...
  // The drilling, in the Details column rather than in columns of its own.
  // Handing and cup positions are three more things to say about a door and
  // this sheet has no room for three more columns: widening it makes every
  // other column narrower on a page that is already full.
  //
  // hingeSummaryLines says nothing at all for a line that is not drilled, so
  // this adds without asking whether there is anything to add.
  hingeSummaryLines(line).forEach((detail) => details.push(detail));
  return details.join("\n") || "-";
}
