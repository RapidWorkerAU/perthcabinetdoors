...
import { calculateQuoteTotals, formatMoney, GST_RATE, toNumber } from "./pcd-quote-utils.js";
...
function wrappedTextLines(value, width, size = 8) {
  ...
  text.split("\n").forEach((paragraph) => {
    ...
    words.forEach((word) => {
      ...
      if (current && textWidth(next, size) > limit) {
        lines.push(current);
        current = word;
        return;
      }
...
function quoteConfigValue(line, key) {
  ...
  if (key === "edge") return String(line.edge_mould || "").trim();
...
/** What the line is called. A hardware line is named, not typed: which hinge. */
function quoteItemName(line) {
  if (isHardwareLine(line)) return String(line.product_name || "").trim() || lineHeading(line);
...
function quoteColumnsFor(group) {
  const shares = [
  ...
  const totalShare = shares.reduce((sum, column) => sum + column.share, 0);
  const scale = QUOTE_CONTENT_WIDTH / totalShare;
  // The last column takes the rounding, so the columns always add up to exactly
  // the width of the page and the right hand rule lands on the border.
  let used = 0;
  return shares.map((column, index) => {
    const width = index === shares.length - 1
      ? QUOTE_CONTENT_WIDTH - used
      : Math.round(column.share * scale);
    used += width;
    return { ...column, width };
  });
}
 ...
 */
function quoteCellLines(line, column) {
  ...
  if (column.key === "item") {
    const out = [{ text: quoteItemName(line), size: QUOTE_FONT, bold: true }];
    const supplied = isHardwareLine(line) ? String(line.supplied_by || "").trim() : "";
    if (supplied && supplied.toLowerCase() !== "we supply") {
      out.push({ text: `Supplied by ${supplied}`, size: QUOTE_NOTE_FONT, muted: true });
    }
    const note = String(line.client_note || "").trim();
    if (note) {
      wrappedTextLines(`Note: ${note}`, column.width - 10, QUOTE_NOTE_FONT).forEach((text) => {
