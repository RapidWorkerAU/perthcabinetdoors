...
import { calculateQuoteTotals, formatMoney, GST_RATE, hingeSpecText, roundMoney, toNumber } from "./pcd-quote-utils.js";
...
import { hasSuggestedSchedule, scheduleDatesLine, scheduleHoldNotice } from "./pcd-quote-schedule.js";
import { depositAfterCredit } from "./pcd-customer-credits.js";
...
function quoteTotalsRows(quote, totals) {
  ...
  //
  // The customer's copy has to say exactly what their screen said, so this is
  // the same pair of lines the public quote page draws, in the same order and
  // from the same stored figure. A credit among the breakdown above would land
  // before GST and hand back more than they paid us. See
  // lib/pcd-customer-credits.js.
  const credit = toNumber(quote.credit_applied_inc_gst);
  const payable = Math.max(0, roundMoney(totals.total_inc_gst - credit));
  if (credit > 0) {
    rows.push({ label: quote.credit_label || "Already paid", value: -credit, ruleBefore: true });
    rows.push({ label: "Amount payable", value: payable, grand: true });
  }

  if (quote.deposit_required) {
    const percent = toNumber(quote.deposit_percent);
    const full = totals.total_inc_gst * (percent / 100);
    // The deposit is a share of the JOB, less money already in. The percentage
    // is still worked out on the full total, because the credit is not a
    // discount on the size of the work.
    const deposit = credit > 0 ? depositAfterCredit(full, credit) : full;
    rows.push({ label: `Deposit on acceptance, ${percent}%`, value: deposit, ruleBefore: true });
    rows.push({ label: "Balance before delivery", value: Math.max(0, roundMoney(payable - deposit)) });
  }
  return rows;
}
...
// The band under the strip that says when the job would happen. Only on a quote
// that suggests dates, so the height it takes has to be worked out per quote
// rather than baked into the first-row position. See quoteFirstTop.
const QUOTE_SCHEDULE_HEIGHT = 40;
 ...
 */
function planQuotePages(blocks, firstTop = QUOTE_FIRST_TOP) {
  const pages = [[]];
  let pageIndex = 0;
  let y = firstTop;
...
/**
 * How far down page one the table can start.
 *
 * A quote with no suggested dates is laid out exactly as it always was, to the
 * point. A quote that has them gives up one band of height for them, and the
 * pagination has to know that BEFORE anything is drawn, or the first group
 * lands underneath the band it does not know about.
 */
function quoteFirstTop(quote) {
  return QUOTE_FIRST_TOP + (hasSuggestedSchedule(quote) ? QUOTE_SCHEDULE_HEIGHT : 0);
}

/**
 * WHEN THE JOB WOULD HAPPEN, on the document they keep.
 *
 * The quote page says this too, and it has to say the same thing: the dates in
 * the same words, and the condition in the same sentence, both out of
 * lib/pcd-quote-schedule.js. A PDF is the copy that gets forwarded and printed
 * and read back to us months later, so it is the one that must not be the
 * version that left the condition off.
 */
function drawQuoteSchedule(page, quote, businessDefaults, y) {
  if (!hasSuggestedSchedule(quote)) return;

  page.fillColor(GREEN_TINT);
  page.strokeColor(GREEN_SOFT);
  page.lineWidth(0.7);
  page.rect(MARGIN, y, QUOTE_CONTENT_WIDTH, QUOTE_SCHEDULE_HEIGHT, { fill: true, stroke: true });

  page.fillColor(MUTED);
  page.text("SUGGESTED DATES", MARGIN + 10, y + 12, QUOTE_LABEL_FONT, { bold: true });

  const datesX = MARGIN + 10 + textWidth("SUGGESTED DATES", QUOTE_LABEL_FONT, true) + 10;
  page.fillColor(STROKE);
  page.text(scheduleDatesLine(quote), datesX, y + 12, 9, { bold: true });

  // The condition, under the dates rather than beside them, because it is a
  // sentence and a sentence next to a value reads as a caption nobody owes
  // anything to.
  page.fillColor(MUTED);
  page.text(scheduleHoldNotice(quote, businessDefaults), MARGIN + 10, y + 27, 7.5);
}
...
function drawQuotePage(page, quote, totals, placedGroups, pageIndex, pageCount, summaryPlacements, businessDefaults) {
  ...
  if (pageIndex === 0) {
    ...
    drawQuoteSchedule(page, quote, businessDefaults, QUOTE_PARTIES_TOP + QUOTE_PARTIES_HEIGHT + QUOTE_STRIP_HEIGHT + QUOTE_STRIP_GAP);
  }
...
export function generateQuotePdf({ quote, lines, businessDefaults, includeCabinetDrawings = true }) {
  ...
  const pdf = new PdfDocument({ logo });
  const quotePlan = planQuotePages(quoteGroupBlocks(quoteGroups(calculatedLines)), quoteFirstTop(quote));
  ...
  for (let pageIndex = 0; pageIndex < quotePageCount; pageIndex += 1) {
    ...
    pdf.addPage((page) => {
      ...
      page.logoWidth = logo?.width || 1;
      page.logoHeight = logo?.height || 1;
      drawQuotePage(page, quote, totals, placedGroups, pageIndex, pageCount, pagePlacements, businessDefaults);
