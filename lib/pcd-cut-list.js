...
import { computeKickboardRun, kickboardSegments, kickboardOffsetMm, kickboardIsInset, hasKickboard, isCornerType } from "./pcd-kickboard-utils";
...
import { computeDoorSizes, computeDoorSizesForConfig, computeDrawerSizes, computeDrawerSizesForConfig, computeCornerDoorLeaves, frontSizingWidthMm, finishedSidePanelDepthMm, finishedTopPanelDepthMm, openBaySections } from "./pcd-door-utils";
import { bottomPanelThicknessMm } from "./pcd-finishpanel-utils";
import { panelReach, panelNeedsKickboard } from "./pcd-panel-options";
import { floatingShelfBoards } from "./pcd-floating-shelf-utils";
import { shelfRailBoards } from "./pcd-shelf-rail-utils";
...
export function computeCutList(item, allItems = [], room = null) {
  ...
  // A Shelf & Rail's cut list is its shelf plus its cleats and front rail —
  // each with the fixing note that matters for that piece (which face to screw
  // into, how far the rail is set back).
  if (item.item_type === "shelf_rail") {
    return shelfRailBoards(item).map((b) => ({
      name: b.label,
      dim1: b.width_mm,
      axis1: b.part === "shelf" ? "W" : "W",
      dim2: b.height_mm,
      axis2: b.part === "shelf" ? "D" : "H",
      material: b.material,
      note: b.note,
    }));
  }
  ...
  if (!isCornerType(item)) {
    ...
    // Shelves inside an OPEN bay of a mixed front. Same board as any other
    // shelf — they were previously not cut at all, so a tall unit with an open
    // shelved bay quoted with no shelves in it.
    openBaySections(item).forEach((bay) => {
      for (let i = 0; i < bay.shelfQty; i++) {
        const suffix = bay.shelfQty === 1 ? "" : ` ${i + 1}`;
        parts.push({
          name: `Shelf${suffix} — bay ${bay.index + 1}`,
          dim1: innerW, axis1: "W", dim2: carcassPanelDepth, axis2: "D",
          material: "shelf",
        });
      }
    });
    ...
    if (item.front_type === "doors") {
      ...
      sections.forEach((sec, idx) => {
        ...
        if (sec.type === "drawers") {
          computeDrawerSizesForConfig(sec.drawer || {}, frontSizingWidthMm(item), sec.height_mm).forEach((size) => {
            const suffix = size.qty > 1 ? ` ×${size.qty}` : "";
          ...
          computeDoorSizesForConfig(sec.door || {}, frontSizingWidthMm(item), sec.height_mm).forEach((size) => {
            const suffix = size.qty > 1 ? ` ×${size.qty}` : "";
  ...
  if (hasKickboard(item)) {
    const kH    = Number(item.kickboard_height_mm) || 120;
    const kSpan = item.kickboard_span || "continuous";
    const isCorner = isCornerType(item);

    // An inset plinth is a rail BETWEEN the sides (bookcase), not a strip under
    // a lifted carcass — named so the bench doesn't cut it full width.
    const kickName = kickboardIsInset(item) ? "Plinth Rail (between sides)" : "Kickboard";
    ...
    if (kSpan === "continuous") {
      ...
      for (const leg of legs) {
        if (leg.count <= 1) {
          ...
          const name = isCorner ? `Kickboard — ${leg.leg === "secondary" ? "Wall 2" : "Wall 1"}` : kickName;
      ...
      for (const seg of kickboardSegments(item, room)) {
        const name = isCorner ? `Kickboard — ${seg.leg === "secondary" ? "Wall 2" : "Wall 1"}` : kickName;
  ...
  if (item.item_type === "wall_cabinet" && item.has_top_panel) {
    const tSpan = item.top_panel_span || "continuous";
    if (tSpan === "continuous") {
      const run = computeTopPanelRun(item, allItems);
      if (run.count <= 1) {
        const widths = splitBackPanelWidths(run.totalWidth, item.top_panel_qty || 1);
        widths.forEach((w, i) => {
          const suffix = widths.length > 1 ? ` ${i + 1}` : "";
          parts.push({ name: `Top Panel${suffix}`, dim1: w, axis1: "W", dim2: finishedTopPanelDepthMm(item) || D, axis2: "D", material: "panel" });
        });
      }
    } else {
      const seg = topPanelSegment(item);
      parts.push({ name: "Top Panel", dim1: seg?.length || topPanelWidthMm(item) || W, axis1: "W", dim2: finishedTopPanelDepthMm(item) || D, axis2: "D", material: "panel" });
    }
  }
  ...
  if (item.item_type === "wall_cabinet" && (item.end_panel_left || item.end_panel_right)) {
    ...
    const sideD = finishedSidePanelDepthMm(item) || D;
    if (item.end_panel_left)  parts.push({ name: "Side Panel — Left",  dim1: sideD, axis1: "D", dim2: sideH, axis2: "H", material: "panel" });
    if (item.end_panel_right) parts.push({ name: "Side Panel — Right", dim1: sideD, axis1: "D", dim2: sideH, axis2: "H", material: "panel" });
  }
  ...
  // floor-standing). A panel set to run to the floor extends down through where
  // a kickboard recess would otherwise be, instead of stopping at carcass
  // height — and each panel decides that for itself, so a left end can run to
  // the floor while the right end stops short.
  if (item.item_type === "base_cabinet" || item.item_type === "tall_cabinet") {
    const panelHFor = (panelKey) => H + (panelReach(item, panelKey).toFloor ? kickboardOffsetMm(item) : 0);
    const panelH = panelHFor("back");

    const sideD = finishedSidePanelDepthMm(item) || D;
    if (item.end_panel_left)  parts.push({ name: "End Panel — Left",  dim1: sideD, axis1: "D", dim2: panelHFor("end_left"), axis2: "H", material: "panel" });
    if (item.end_panel_right) parts.push({ name: "End Panel — Right", dim1: sideD, axis1: "D", dim2: panelHFor("end_right"), axis2: "H", material: "panel" });
    ...
    if (hasKickboard(item)) {
      ...
      if (item.end_panel_left  && panelNeedsKickboard(item, "end_left"))  parts.push({ name: "Kickboard — Left End",  dim1: D, axis1: "D", dim2: kH2, axis2: "H", material: "kickboard" });
      if (item.end_panel_right && panelNeedsKickboard(item, "end_right")) parts.push({ name: "Kickboard — Right End", dim1: D, axis1: "D", dim2: kH2, axis2: "H", material: "kickboard" });
  ...
  if (isCornerType(item) && (item.back_panel_wall1 || item.back_panel_wall2)) {
    ...
    if (item.back_panel_wall1) parts.push({ name: "Back Panel — Wall 1", dim1: W, axis1: "W", dim2: panelHFor("back_wall1"), axis2: "H", material: "panel" });
    if (item.back_panel_wall2 && item.secondary_wall && secW > 0) {
      parts.push({ name: "Back Panel — Wall 2", dim1: secW, axis1: "W", dim2: panelHFor("back_wall2"), axis2: "H", material: "panel" });
    }

    if (hasKickboard(item)) {
      const kH2 = Number(item.kickboard_height_mm) || 120;
      if (item.back_panel_wall1 && panelNeedsKickboard(item, "back_wall1")) parts.push({ name: "Kickboard — Wall 1 Back", dim1: W, axis1: "W", dim2: kH2, axis2: "H", material: "kickboard" });
      if (item.back_panel_wall2 && item.secondary_wall && secW > 0 && panelNeedsKickboard(item, "back_wall2")) {
