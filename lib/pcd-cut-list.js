...
import { computeKickboardRun, kickboardSegments, kickboardOffsetMm, kickboardIsInset, isCornerType } from "./pcd-kickboard-utils";
...
export function computeCutList(item, allItems = [], room = null) {
  ...
  if (item.item_type === "base_cabinet" || item.item_type === "tall_cabinet") {
    ...
    const sideD = finishedSidePanelDepthMm(item) || D;
    if (item.end_panel_left)  parts.push({ name: "End Panel — Left",  dim1: sideD, axis1: "D", dim2: panelH, axis2: "H", material: "panel" });
    if (item.end_panel_right) parts.push({ name: "End Panel — Right", dim1: sideD, axis1: "D", dim2: panelH, axis2: "H", material: "panel" });
