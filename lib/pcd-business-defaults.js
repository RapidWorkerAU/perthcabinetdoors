...
//
// hinge_supply_unit_cost_ex_gst went the same way on 21 September 2026. It was
// pinned to 0 in the built-in defaults, pinned to 0 again in the normaliser,
// written to the database as 0, shown on no screen and read by no pricing code:
// three lines and a column that between them did nothing. Hinges are supplied
// as hardware lines now. Its column keeps whatever it holds.
export function businessDefaultsToDbRow(defaults = {}) {
  ...
  return {
    ...
    quote_valid_days: normalized.quote_valid_days,
    schedule_hold_hours: normalized.schedule_hold_hours,
    ...
    saw_kerf_mm: normalized.saw_kerf_mm,
    board_edge_trim_mm: normalized.board_edge_trim_mm,
    web_delivery_metro_ex_gst: normalized.web_delivery_metro_ex_gst,
  };
}
...
/**
 * How long a quote stands for, as configured.
 *
 * HERE RATHER THAN WITH THE EXPIRY JOB, because three unrelated things read it
 * and one of them is a read only report. Asking the job for it would drag the
 * mail client into the reporting route to answer a question about a setting.
 *
 * Never throws and never returns nothing: a settings row that cannot be read
 * falls back to what our terms have always said, because the alternative is a
 * job that either expires everything or expires nothing.
 */
export async function quoteValidDays(supabase) {
  try {
    const defaults = await getBusinessDefaults(supabase);
    const configured = Number(defaults?.quote_valid_days);
    return Number.isFinite(configured) && configured > 0
      ? configured
      : DEFAULT_BUSINESS_DEFAULTS.quote_valid_days;
  } catch {
    return DEFAULT_BUSINESS_DEFAULTS.quote_valid_days;
  }
}
