...
//
//   A PRODUCTION RUN is derived. Both its dates already live on the order, as
//   scheduled_start_date and target_completion_date, typed by whoever scheduled
//   the job, see pcd-order-schedule.js. Nothing about a run is
...
import { durationDays } from "./pcd-order-schedule";
 ...
 *
 * Both ends are read straight off the order. They used to be a start date and a
 * timeframe, with the end worked out here so the calendar and the order screen
 * could not disagree; now they are two typed dates and there is nothing left to
 * work out. An order with a completion date and no start is still drawn, as a
 * single day marker on the day it is due, because a job with a promised date
 * and no plan still has to be visible.
 */
export function runFromOrder(order) {
  ...
  return {
    ...
    // How many days the bar covers, counting the start day itself.
    leadDays: durationDays({ scheduled_start_date: start, target_completion_date: end }) || 1,
    labourHours: Number(order.labour_hours) || 0,
    scheduled: true,
  };
}
...
/**
 * What a booking that would not save says to the person who tried to make it.
 *
 * A database that refuses a row says so in its own words, and "violates foreign
 * key constraint pcd_calendar_events_order_id_fkey" is not something anybody
 * booking a measure can act on. The three links a booking can carry are the
 * only ones that can be refused this way, so each gets a sentence saying which
 * one has gone and what to do about it.
 */
export function bookingSaveMessage(error) {
  const text = String(error?.message || "");
  if (error?.code === "23503" || text.includes("foreign key constraint")) {
    if (text.includes("order_id")) return "The job this was being booked against is no longer there. Pick the job again and save it.";
    if (text.includes("quote_id")) return "The quote this was being booked against is no longer there. Pick the job again and save it.";
    if (text.includes("customer_id")) return "That customer is no longer there. Pick the customer again and save it.";
    return "Something this booking was linked to is no longer there. Pick the customer and job again and save it.";
  }
  return text || "Could not save the booking.";
}
