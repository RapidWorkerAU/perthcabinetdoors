...
export function buildColourFamilyFromLibraryRows({ rows = [], material, thickness = "" }) {
  ...
  const activeRows = rows
    ...
    .sort((a, b) => {
      if (thicknessValue) {
        const aExactThickness = String(a.thickness || "").trim().toLowerCase() === thicknessValue;
        const bExactThickness = String(b.thickness || "").trim().toLowerCase() === thicknessValue;
        if (aExactThickness !== bExactThickness) return aExactThickness ? -1 : 1;
      }
