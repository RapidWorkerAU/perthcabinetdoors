...
export function buildColourFamilyFromLibraryRows({ rows = [], material, thickness = "" }) {
  ...
  const thicknessValue = String(thickness || inferThicknessFromMaterial(material)).trim();

  const activeRows = rows
    .filter((row) => row.is_active !== false)
    .filter((row) => String(row.material_type || "").toLowerCase() === materialType.toLowerCase())
    .filter((row) => !thicknessValue || String(row.thickness || "").toLowerCase() === thicknessValue.toLowerCase())
    .sort((a, b) => {
...
export async function getDatabaseColourFamilyForSelection(supabase, { material, thickness = "" }) {
  ...
  if (thicknessValue) {
    query = query.eq("thickness", thicknessValue);
  }
