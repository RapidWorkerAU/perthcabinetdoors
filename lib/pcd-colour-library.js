...
const COLOUR_TILE_BUCKET = "colour-tiles";
const COLOUR_TILE_SIGNED_URL_SECONDS = 60 * 60 * 6;
...
async function rowsWithFreshTileUrls(supabase, rows = []) {
  return Promise.all((rows || []).map(async (row) => {
    const imagePath = String(row.image_path || "").trim();
    if (!imagePath) return row;
    try {
      const { data, error } = await supabase.storage
        .from(COLOUR_TILE_BUCKET)
        .createSignedUrl(imagePath, COLOUR_TILE_SIGNED_URL_SECONDS);
