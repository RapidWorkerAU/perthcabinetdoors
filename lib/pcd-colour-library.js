import { MATERIALS, THICKNESS_BY_DESIGN, normaliseMaterialKey } from "./pcd-materials";

// Re-derived from lib/pcd-materials.js (single source of truth). COLOUR_MATERIALS
// keeps its historical { key, label, value } shape — `value` is the design-tool
// lowercase spelling of the material.
export const COLOUR_MATERIALS = MATERIALS.map((m) => ({ key: m.key, label: m.label, value: m.design }));

export const COLOUR_THICKNESS_BY_MATERIAL = THICKNESS_BY_DESIGN;

export const COLOUR_ORDER_TYPES = [
  { value: "supply board", label: "Supply board" },
  { value: "made to order MTO", label: "Made to order MTO" },
];

// THE BOARD BRANDS WE STOCK, and the one list of how each is spelt.
//
// Adding one here does three things at once: it appears in the Supplier
// dropdown on a colour, it becomes a filter on the library, and
// normaliseSupplierName below starts returning it in our spelling whatever case
// it arrives in. Nothing else needs a change, because everything downstream
// reads the brands off the colours themselves rather than from a list in code.
// See lib/pcd-supplier-selection.js.
export const COLOUR_SUPPLIERS = ["Polytec", "Laminex", "Formica", "Paperock"];

// THE FINISHES A BOARD COMES IN, AND WHY THEY ARE A LIST.
//
// This was a free text box on the colour library with "e.g. Woodmatt" in it,
// and free text is how one finish becomes two. The live library grew:
//
//     "Absolute Grain" and "AbsoluteGrain"
//     "Absolute Matt"  and "AbsoluteMatte"
//     "Raw"            and "Raw Finish"
//
// Six rows, three finishes, and every screen that groups or filters by finish
// treated each spelling as its own thing: the /finishes page listed it twice,
// the picker offered it twice, and a quote could be written against either.
//
// Seeded from what the library actually holds today, minus the duplicates. It
// is a Settings, Lists vocabulary, so a new finish is added there or with the
// Add new box on the colour itself, rather than by typing a word and hoping it
// matches what somebody typed last time.
//
// DELIBERATELY NOT PER BRAND. A finish belongs to a brand in the real world,
// and holding them apart here would mean a customer's Matt could not be found
// because it was filed under Polytec while the board was Laminex. The brand is
// already on the colour; this is the vocabulary of words used across all of
// them.
export const COLOUR_FINISHES = [
  "Absolute Grain",
  "Absolute Matt",
  "Ashgrain",
  "Flint",
  "Gloss",
  "Legato",
  "Matt",
  "Natura",
  "Natural",
  "Pearl",
  "Ravine",
  "Raw",
  "Satin",
  "Sheen",
  "Silk",
  "Smooth",
  "Tactile",
  "Texture",
  "Ultramatt",
  "Velvet",
  "Venette",
  "Woodgrain",
  "Woodmatt",
];

/**
 * Whether a colour's image_url is actually a picture of the board.
 *
 * NOT EVERY image_url IS AN IMAGE. Some rows carry a link to the supplier's
 * product PAGE instead, which is the same thing that used to flatten the 3D
 * room. A page URL renders nothing: in a run of tiles it is a transparent gap
 * in the middle of the band, and on a drawn front it is a door with no board on
 * it. So anywhere a tile is shown as an image, it has to be one of ours or at
 * least end in something that is a picture.
 *
 * This is a display guard, not a fix. The rows themselves want cleaning up in
 * the colour library.
 */
export function isColourTileUrl(url) {
  const value = String(url || "");
  return /\/storage\/v1\/object\//.test(value) || /\.(jpe?g|png|webp|avif)(\?|$)/i.test(value);
}

/**
 * A brand name as it should be spelt, wherever it came from.
 *
 * The library held "formica" in lower case, so it reached the finishes page, the
 * configurator and the quote form looking like a typo, and two public pages had
 * each written their own little title-caser to paper over it. Worse, the admin
 * colour editor checks the stored value against COLOUR_SUPPLIERS and falls back
 * to Polytec when it does not match, so opening that row for editing would have
 * quietly re-saved a Formica board as a Polytec one.
 *
 * Applied on the way in and on the way out. A brand we know is returned exactly
 * as we spell it, whatever case it arrives in. One we do not know is title-cased
 * word by word rather than thrown away, because a brand we have not listed yet
 * is somebody's real supplier, not a mistake.
 */
// `brands` is the live list from Settings, Lists where the caller has it. The
// name typed there IS the spelling, so a brand written "IKEA" has to come back
// as "IKEA" rather than being title-cased into "Ikea" by the fallback below.
export function normaliseSupplierName(value, brands = []) {
  const name = String(value ?? "").trim();
  if (!name) return "";
  const added = (brands || []).filter((brand) => String(brand?.key || "").toLowerCase() === name.toLowerCase())[0];
  if (added) return added.key;
  const known = COLOUR_SUPPLIERS.find((supplier) => supplier.toLowerCase() === name.toLowerCase());
  if (known) return known;
  return name
    .split(/\s+/)
    .map((word) => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase())
    .join(" ");
}

/**
 * A board written out the way a person reads it back: who makes it, what colour
 * it is, what finish, how thick.
 *
 *   "Polytec Black Wenge Matt 16mm"
 *
 * The colour alone is not enough to check a default by. Two brands stock a
 * Black, the same colour comes in three finishes at different money, and the
 * thickness is the thing most worth catching before a whole job is cut from it.
 * Anything missing is simply left out rather than shown as a gap.
 *
 * One speller, used by every place that shows a board in one line, so the
 * summary on a collapsed section and the summary inside the picker cannot word
 * the same board two ways.
 */
export function boardSummary(style, brands = []) {
  const s = style || {};
  const supplier = normaliseSupplierName(s.supplier || s.supplier_name, brands);
  const name = String(s.colour || "").trim();
  const finish = String(s.finish || "").trim();
  const thickness = Number(s.thickness_mm) || 0;
  // A board with no colour picked is described by its material, if it has one:
  // "18mm decorative board" still tells you something. Nothing at all is blank,
  // and the caller decides what to say instead.
  if (!name) {
    const material = String(s.material || "").trim();
    if (!material) return "";
    return [thickness ? `${thickness}mm` : "", material].filter(Boolean).join(" ");
  }
  return [supplier, name, finish, thickness ? `${thickness}mm` : ""].filter(Boolean).join(" ");
}

export function normaliseOrderTypes(rowOrTypes) {
  if (Array.isArray(rowOrTypes)) {
    return rowOrTypes.filter(Boolean);
  }
  if (Array.isArray(rowOrTypes?.order_types)) {
    return rowOrTypes.order_types.filter(Boolean);
  }
  return rowOrTypes?.order_type ? [rowOrTypes.order_type] : ["supply board"];
}

// WHETHER A BOARD IS MADE TO ORDER, which is the difference between a colour we
// have not priced and a colour that is not priced this way.
//
// Decorative board is board we buy and cut ourselves, so it is held per m² and
// a blank cost on one is a real gap. Thermolaminate and compact laminate are
// made to order: the job goes to the supplier and comes back with a price for
// that job, so there is nothing to hold per m² and there never will be.
//
// The whole system used to test the PRICE instead, so every MTO colour read as
// a fault: 236 "unpriced" colours on the library screen, "no board cost" on
// every thermolaminate quote line, a reprice reporting lines it could never
// have repriced, and a pre-flight warning on every design import. Five warnings
// about normal business, which is how people learn to ignore the one that
// matters.
export function isMadeToOrder(rowOrTypes) {
  return normaliseOrderTypes(rowOrTypes).some((type) => String(type).toLowerCase().includes("made to order"));
}

// A colour we would hold a per-m² cost for and do not. Made-to-order colours are
// not counted: they are quoted from the supplier, not from a rate.
export function isMissingBoardPrice(row) {
  return !isMadeToOrder(row) && !(Number(row?.cost_per_sqm_ex_gst || 0) > 0);
}

export function orderTypesLabel(rowOrTypes) {
  const values = normaliseOrderTypes(rowOrTypes);
  return values
    .map((value) => COLOUR_ORDER_TYPES.find((type) => type.value === value)?.label || value)
    .join(", ");
}

export function normaliseColourMaterialKey(material) {
  return normaliseMaterialKey(material);
}

export function materialTypeForKey(material) {
  const key = normaliseColourMaterialKey(material);
  return COLOUR_MATERIALS.find((item) => item.key === key)?.value || String(material || "");
}

export function materialLabelForType(materialType) {
  const key = normaliseColourMaterialKey(materialType);
  return COLOUR_MATERIALS.find((item) => item.key === key)?.label || String(materialType || "");
}

export function colourFamilyLabel(material) {
  const key = normaliseColourMaterialKey(material);
  if (key === "thermolaminate") return "Thermolaminate";
  if (key === "compact_laminate") return "Compact Laminate";
  return materialTypeForKey(key) || "Decorative board";
}

export function colourFamilyNote(material) {
  const key = normaliseColourMaterialKey(material);
  if (key === "thermolaminate") {
    return "Thermolaminate colours suitable for wrapped door and drawer front profiles.";
  }
  if (key === "compact_laminate") {
    return "Compact laminate colours for hard-wearing tops, wardrobe doors and wet-area surfaces.";
  }
  return "Decorative board colours for 16mm and 18mm board products.";
}

export function thicknessOptionsForMaterial(material) {
  const materialType = materialTypeForKey(material);
  return COLOUR_THICKNESS_BY_MATERIAL[materialType] || [];
}

export function inferThicknessFromMaterial(material) {
  const value = String(material || "").toLowerCase();
  if (value.includes("21")) return "21mm";
  if (value.includes("18")) return "18mm";
  if (value.includes("16")) return "16mm";
  if (value.includes("13")) return "13mm";
  if (value.includes("5")) return "5mm";
  return "";
}

export function optionsFromColourFamily(colourFamily) {
  return (colourFamily?.groups || []).flatMap((group) =>
    (group.colours || []).map((colour) => ({
      id: colour.id,
      finish: group.label,
      name: colour.name,
      value: [colour.supplier, group.label, colour.name, colour.thickness].filter(Boolean).join("::"),
      thickness: colour.thickness || "",
      src: colour.src,
      supplier: colour.supplier || "",
      orderType: colour.orderType || "",
      orderTypes: colour.orderTypes || normaliseOrderTypes(colour.orderType ? [colour.orderType] : []),
      costPerBoardExGst: colour.costPerBoardExGst ?? 0,
      costPerSqmExGst: colour.costPerSqmExGst ?? 0,
      preferredBoardWidthMm: colour.preferredBoardWidthMm ?? 0,
      preferredBoardHeightMm: colour.preferredBoardHeightMm ?? 0,
      label: [group.label, colour.name, colour.thickness].filter(Boolean).join(" - "),
    }))
  );
}

/**
 * What picking a colour option does to a quote line.
 *
 * There are two colour comboboxes: the quote editor's own, and the shared one
 * the order-variation editor uses. They are different widgets (one is a Radix
 * popover, the other a hand-positioned portal) and that is fine, but what they
 * WRITE has to be identical or a line means different things depending on which
 * screen made it. This is that shared contract.
 *
 * Note what is deliberately absent: any fallback to option.label. The label
 * reads "Finish - Colour - Thickness", so falling back to it on a library row
 * with a blank name would push all three into the colour column, which is the
 * exact fault the website quote form used to create.
 */
export function colourSelectionPatch(option, fallbackSupplier = "") {
  const supplier = normaliseSupplierName(option.supplier || fallbackSupplier);
  return {
    supplier_name: supplier,
    thickness: option.thickness || "",
    colour: option.name || "",
    finish: option.finish || "",
    unit_cost_source_id: option.id || null,
    unit_cost_source_label: [supplier, option.label || option.name].filter(Boolean).join(" - "),
    unit_cost_per_sqm_ex_gst: Number(option.costPerSqmExGst || 0),
    cost_per_board_ex_gst: Number(option.costPerBoardExGst || 0),
    preferred_board_width_mm: Number(option.preferredBoardWidthMm || 0),
    preferred_board_height_mm: Number(option.preferredBoardHeightMm || 0),
  };
}

export function buildColourFamilyFromLibraryRows({ rows = [], material, thickness = "" }) {
  const materialType = materialTypeForKey(material);
  if (!materialType) return null;
  const materialKey = normaliseColourMaterialKey(materialType);
  const thicknessValue = String(thickness || inferThicknessFromMaterial(material)).trim().toLowerCase();

  const activeRows = rows
    .filter((row) => row.is_active !== false)
    .filter((row) => normaliseColourMaterialKey(row.material_type) === materialKey)
    .filter((row) => {
      if (!thicknessValue) return true;
      return String(row.thickness || "").trim().toLowerCase() === thicknessValue;
    })
    .sort((a, b) => {
      const finishSort = String(a.finish_type || "").localeCompare(String(b.finish_type || ""));
      if (finishSort) return finishSort;
      const orderSort = Number(a.sort_order || 0) - Number(b.sort_order || 0);
      if (orderSort) return orderSort;
      return String(a.name || "").localeCompare(String(b.name || ""));
    });

  const finishMap = new Map();
  activeRows.forEach((row) => {
    const finish = String(row.finish_type || "Unspecified").trim() || "Unspecified";
    if (!finishMap.has(finish)) {
      finishMap.set(finish, []);
    }
    const colours = finishMap.get(finish);
    const colourName = String(row.name || "").trim();
    const rowThickness = String(row.thickness || "").trim();
    const existingIndex = colours.findIndex((colour) => {
      const sameName = String(colour.name || "").trim().toLowerCase() === colourName.toLowerCase();
      const sameThickness = String(colour.thickness || "").trim().toLowerCase() === rowThickness.toLowerCase();
      return sameName && sameThickness;
    });
    const nextColour = {
      id: row.id,
      name: colourName,
      thickness: rowThickness,
      src: row.image_url || "",
      imagePath: row.image_path || "",
      supplier: normaliseSupplierName(row.supplier_name) || "Polytec",
      orderType: row.order_type || "supply board",
      orderTypes: normaliseOrderTypes(row),
      costPerBoardExGst: Number(row.cost_per_board_ex_gst || 0),
      costPerSqmExGst: Number(row.cost_per_sqm_ex_gst || 0),
      preferredBoardWidthMm: Number(row.preferred_board_width_mm || 0),
      preferredBoardHeightMm: Number(row.preferred_board_height_mm || 0),
    };

    if (existingIndex === -1) {
      colours.push(nextColour);
      return;
    }

    if (!colours[existingIndex].src && nextColour.src) {
      colours[existingIndex] = nextColour;
    }
  });

  const groups = Array.from(finishMap.entries()).map(([label, colours]) => ({
    label,
    colours,
  }));

  return {
    label: colourFamilyLabel(material),
    note: colourFamilyNote(material),
    groups,
  };
}

export function buildColourAvailabilityFromLibraryRows(rows = []) {
  const availability = {};

  rows
    .filter((row) => row.is_active !== false)
    .forEach((row) => {
      const materialKey = normaliseColourMaterialKey(row.material_type);
      const thickness = String(row.thickness || "").trim();
      if (!materialKey || !thickness) return;
      if (!availability[materialKey]) availability[materialKey] = [];
      if (!availability[materialKey].includes(thickness)) {
        availability[materialKey].push(thickness);
      }
    });

  Object.keys(availability).forEach((materialKey) => {
    const preferredOrder = COLOUR_THICKNESS_BY_MATERIAL[materialTypeForKey(materialKey)] || [];
    availability[materialKey].sort((a, b) => {
      const aIndex = preferredOrder.indexOf(a);
      const bIndex = preferredOrder.indexOf(b);
      if (aIndex !== -1 || bIndex !== -1) {
        if (aIndex === -1) return 1;
        if (bIndex === -1) return -1;
        return aIndex - bIndex;
      }
      return a.localeCompare(b, undefined, { numeric: true });
    });
  });

  return availability;
}

export async function getDatabaseColourRows(supabase, { activeOnly = false, throwOnError = false } = {}) {
  let query = supabase
    .from("pcd_colour_library")
    .select("*")
    .order("material_type", { ascending: true })
    .order("thickness", { ascending: true })
    .order("finish_type", { ascending: true })
    .order("sort_order", { ascending: true })
    .order("name", { ascending: true });

  if (activeOnly) {
    query = query.eq("is_active", true);
  }

  const { data, error } = await query;
  if (error) {
    if (throwOnError) throw error;
    return [];
  }
  return data || [];
}

export async function getDatabaseColourFamily(supabase, material) {
  return getDatabaseColourFamilyForSelection(supabase, { material });
}

function proxiedColourTileUrl(imagePath) {
  return `/api/colour-library/tile?path=${encodeURIComponent(imagePath)}`;
}

// A tile linked straight to a supplier's website goes through the same proxy.
// Not for tidiness: the design tool's 3D view loads a tile as a WebGL texture,
// which is a CORS fetch, and supplier sites send no CORS header. One texture
// failing drops the WHOLE 3D room back to flat colours, so a single linked
// colour anywhere in a job made the colour toggle look broken. Served from our
// own origin it behaves like every uploaded tile. See the tile route for the
// list of sites we will fetch from.
function proxiedRemoteTileUrl(imageUrl) {
  return `/api/colour-library/tile?url=${encodeURIComponent(imageUrl)}`;
}

async function rowsWithFreshTileUrls(_supabase, rows = []) {
  return Promise.all((rows || []).map(async (row) => {
    const imagePath = String(row.image_path || "").trim();
    if (imagePath) return { ...row, image_url: proxiedColourTileUrl(imagePath) };
    const imageUrl = String(row.image_url || "").trim();
    if (imageUrl.toLowerCase().startsWith("https://")) return { ...row, image_url: proxiedRemoteTileUrl(imageUrl) };
    return row;
  }));
}

// Every active library row as one flat "item" — the shared colour picker shows
// one tile per item, so each thickness variant is its own tile (with its own
// cost). `material` is the design-tool value (for storage); `materialLabel` is
// for display. This is the single source of truth for cost — it is never edited
// in the design tool.
export async function getDatabaseColourItems(supabase) {
  const rows = await rowsWithFreshTileUrls(supabase, await getDatabaseColourRows(supabase, { activeOnly: true }));
  return (rows || [])
    .map((row) => {
      const thickness = String(row.thickness || "").trim();
      return {
        id: row.id,
        material: materialTypeForKey(row.material_type),
        materialLabel: materialLabelForType(row.material_type),
        thickness,
        thicknessMm: parseInt(thickness, 10) || null,
        finish: String(row.finish_type || "Unspecified").trim() || "Unspecified",
        colour: String(row.name || "").trim(),
        supplier: normaliseSupplierName(row.supplier_name) || "Polytec",
        src: row.image_url || "",
        cost: Number(row.cost_per_sqm_ex_gst || 0),
        // How this board is bought. A made-to-order board has no cost per m²
        // and never will, so a reader has to be able to tell that apart from a
        // colour we simply have not priced yet. See isMadeToOrder.
        orderTypes: normaliseOrderTypes(row),
        madeToOrder: isMadeToOrder(row),
        // WHAT SIZE THE BOARD IS AND WHETHER IT HAS A GRAIN.
        //
        // Both belong to the colour rather than to a quote, and both were
        // already being dropped here: the board size has been on the row since
        // the August 2026 price update and nothing outside the library screen
        // could see it, so the board order had no choice but to assume 1200 x
        // 2400 for every colour. A zero means the row has never been given a
        // size, which is a different thing from 1200 x 2400 and has to stay
        // tellable apart. See lib/pcd-board-order.js.
        boardWidthMm: Number(row.preferred_board_width_mm || 0),
        boardHeightMm: Number(row.preferred_board_height_mm || 0),
        // Null until somebody answers it on the Board Library screen. The
        // board order then guesses from the finish name and says that it did,
        // rather than quietly treating an unanswered colour as plain.
        hasGrain: typeof row.has_grain === "boolean" ? row.has_grain : null,
      };
    })
    .filter((it) => it.colour);
}

function stripFinishPrefix(colour, finish) {
  const rawColour = String(colour || "").trim();
  const finishLabel = String(finish || "").trim();
  if (finishLabel && rawColour.toLowerCase().startsWith(`${finishLabel.toLowerCase()} - `)) {
    return rawColour.slice(finishLabel.length + 3).trim();
  }
  return rawColour;
}

export function findColourInFamily(colourFamily, { finish = "", colour = "" } = {}) {
  const cleanedColour = stripFinishPrefix(colour, finish).toLowerCase();
  if (!cleanedColour || !colourFamily?.groups?.length) return null;

  const finishLabel = String(finish || "").trim().toLowerCase();
  const groups = finishLabel
    ? colourFamily.groups.filter((group) => String(group.label || "").trim().toLowerCase() === finishLabel)
    : colourFamily.groups;

  for (const group of groups) {
    const match = (group.colours || []).find((item) => String(item.name || "").trim().toLowerCase() === cleanedColour);
    if (match) return { finish: group.label, colour: match };
  }
  return null;
}

export async function getDatabaseColourFamilyForSelection(supabase, { material, thickness = "" }) {
  const materialType = materialTypeForKey(material);
  if (!materialType) return null;

  const { data, error } = await supabase
    .from("pcd_colour_library")
    .select("*")
    .eq("is_active", true)
    .order("finish_type", { ascending: true })
    .order("sort_order", { ascending: true })
    .order("name", { ascending: true });

  if (error) return null;

  return buildColourFamilyFromLibraryRows({
    rows: await rowsWithFreshTileUrls(supabase, data || []),
    material,
    thickness,
  });
}

export async function getDatabaseColourSuppliers(supabase) {
  const { data, error } = await supabase
    .from("pcd_colour_library")
    .select("id,name,finish_type,supplier_name,is_active")
    .eq("is_active", true)
    .order("finish_type", { ascending: true })
    .order("name", { ascending: true });

  if (error) return [];

  return (data || []).map((row) => ({
    id: row.id,
    finish: row.finish_type || "",
    name: row.name,
    supplier: normaliseSupplierName(row.supplier_name) || "Polytec",
    label: [row.finish_type, row.name].filter(Boolean).join(" - "),
  }));
}
