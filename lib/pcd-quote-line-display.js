...
import { bandedEdgesText, panelUseFor } from "./pcd-line-details";
...
export function lineHeading(line) {
  ...
  // A panel is named by what it IS. "Scribe" and "Kickboard" are what the
  // customer asked for and is checking the quote for; six rows all reading
  // "Panel" is the same problem the hardware kind above was fixing.
...
/**
 * The edges column for a customer: the edge profile, then which edges are
 * banded. Either can be missing. Returned as separate lines so each screen can
 * lay them out its own way.
 */
export function lineEdgeLines(line) {
  return [text(line?.edge_mould), bandedEdgesText(line?.banded_edges)].filter(Boolean);
}
