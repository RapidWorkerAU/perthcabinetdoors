...
import { retrieveCheckoutSession, siteUrl } from "./pcd-stripe";
import { markCheckoutExpired, AWAITING_DEPOSIT } from "./pcd-deposit-gate";
// finaliseDepositAcceptance, through the step that also tells sales@ and the
// customer when this pass is the one that made the order.
import { completeGateSession } from "./pcd-gate-complete";
import {
  ...
  for (const checkout of open || []) {
    ...
    try {
      ...
      if (session?.payment_status === "paid") {
        ...
        const result = await completeGateSession(supabase, session, { baseUrl: siteUrl() });
