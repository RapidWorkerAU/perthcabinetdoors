...
// Every panel slot a design item can carry a colour on. Used to audit a room
// for colours the library has no tile image for.
const AUDIT_SLOTS = [
  "carcass", "door", "drawer", "endpanel_left", "endpanel_right",
  "filler", "kickboard", "top", "underside", "back", "shelf", "benchtop",
];

/**
 * The colour SELECTIONS used across `items` that the library has no tile image
 * for, so "show colours" leaves those panels on their flat type colour.
 *
 * Why this exists: a library row with no image is invisible everywhere. A whole
 * job can be drawn in one colour that happens to have no tile, and then the
 * colour toggle looks broken. Nothing changes when you press it, except the
 * one or two cabinets in a colour that DOES have a tile, which then read as
 * "the wrong colours". This names the gap instead of failing silently. Fixing
 * it is an upload on the Board Library screen, not a code change.
 *
 * Returns [{ material, finish, colour, label }], one per distinct selection.
 */
export function unresolvedColourSelections(map, items) {
  if (!map) return [];
  const out = new Map();
  for (const item of items || []) {
    if (!item) continue;
    for (const slot of AUDIT_SLOTS) {
      const { material, finish, colour } = slotColourFields(item, slot);
      if (!material || !colour) continue;
      if (slot === "carcass" && item.prop_carcass_finish) continue;
      if (map.byFull?.get(colourKey(material, finish, colour))) continue;
      if (map.byLoose?.get(colourKeyLoose(material, colour))) continue;
      const key = colourKey(material, finish, colour);
      if (!out.has(key)) {
        out.set(key, { material, finish, colour, label: finish ? `${colour} · ${finish}` : colour });
      }
    }
  }
  return [...out.values()];
