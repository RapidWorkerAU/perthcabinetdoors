...
//
// ── NOTHING WRITES TO THIS ANY MORE ──────────────────────────────────────────
//
// The IKEA configurator was its only producer and it has been removed: that
// page is now written content, and the builder on /request-quote is where
// anybody specifies a piece. This file stays because somebody who configured a
// list before the change still has it in their browser, and the drain in
// RequestQuoteFormClient is what rescues it into their real list rather than
// losing it silently. The nav counts it for the same reason.
//
// It can go once nobody could plausibly still be holding one. Delete the store,
// the drawer, and the drain together, not one of the three.
  ...
  list.forEach((entry) => {
    ...
    if (entry.kind === "custom") {
      lines.push({
        ...
        colour: entry.colour || "",
        colourSrc: "",
        colourLibraryId: entry.colourLibraryId || "",
