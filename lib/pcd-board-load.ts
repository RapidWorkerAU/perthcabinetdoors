import { buildBoard, collapseReplies, daysUntil, requestAnswered } from './pcd-board'
import { applyDismissals } from './pcd-board-dismissal'
...
//
// ── WHY THIS IS NOT IN THE BOARD PAGE ANY MORE ───────────────────────────────
//
// It was, and then the dashboard grew a second list of the same work built from
// its own queries. That list did not know about set aside, so a card somebody
// had deliberately cleared off the board reappeared on the dashboard an hour
// later, which is worse than not having the list at all: a board you have
// cleared and a dashboard that disagrees means neither can be trusted.
//
// So there is one definition of what needs doing. The board shows all of it,
// grouped. The dashboard ranks it and shows the top of it. Both read this.
//
// ── WHAT IT RETURNS ──────────────────────────────────────────────────────────
//
//   cards          what is standing, after set aside and after collapsing
//                  replies. Already in board card shape.
//   setAsideCount  how many were hidden, so a screen can say so.
//   failed         which SOURCES would not load. A failed query returns no rows
//                  and no rows reads as all clear, so the caller has to be able
//                  to say a column is unloaded rather than empty.
//   today          the date in Perth, which every age is measured from.

type Json = Record<string, unknown>

// The real client's type rather than a hand written one, so the row shapes the
// queries below infer are the same ones they inferred when this lived in the
// page. A loosely typed client turns every row into unknown and quietly takes
