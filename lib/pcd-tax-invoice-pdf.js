...
function rowHeight(line) {
  return Math.max(26, wrap(line.description, DESCRIPTION_WIDTH, 9).length * 12 + 14);
}
...
function drawMasthead(page, invoice, businessDefaults) {
  ...
  const business = [
    "Perth Cabinet Doors is a trading",
    "entity under AL & JP Pty Ltd.",
    "",
    "Email:",
    businessDefaults.email || "sales@perthcabinetdoors.com.au",
    `ABN: ${businessDefaults.abn || "27617956215"}`,
...
function drawRow(page, line, rowTop, gstRate) {
  const wrapped = wrap(line.description, DESCRIPTION_WIDTH, 9);
  const height = rowHeight(line);
  const firstBaseline = centredBaseline(rowTop, height, wrapped.length, 9);

  page.fillColor(INK);
  wrapped.forEach((part, index) =>
    page.text(part, BANDS.description.left, firstBaseline + index * LINE_GAP, 9)
  );
...
function drawFooter(page, invoice, top, businessDefaults) {
  ...
  page.fillColor(MUTED);
  const contact = businessDefaults.email || "sales@perthcabinetdoors.com.au";
  [
    `For accounts enquiries, please contact ${businessDefaults.accountsContact || "Jason Phillips"} at ${contact}.`,
