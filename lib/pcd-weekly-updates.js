...
export const CUSTOMER_FACING_ACTIONS = [
  ...
  "variation_applied",
  // A variation the customer approved but which has not been applied yet is
  // still news to them: they pressed the button and want to know it landed.
  "variation_approved",
  "issue_raised",
  // A delivery or an install being booked is the single thing a customer most
  // wants to hear, and it was the one event this report could not see.
  "order_booking_updated",
];
...
function changesFromRow(row, itemsById) {
  ...
  if (row.action_type === "variation_applied") {
    // NO CHARGE IS NOT AN AMOUNT, AND MUST NOT BE READ AS ONE.
    //
    // An admin override approves a size change and holds the price, and the
    // activity row still names the figure, because internally somebody needs to
    // know what was given away. That figure is NOT owed by anybody. Read as an
    // amount it would go out in an email telling the customer their order had
    // gone up by a sum nobody is charging them, which is the worst kind of
    // wrong number: it is specific, it is in writing, and it is a bill.
    //
    // So the flag is read off the metadata, which is where the applier writes
    // it, rather than inferred from the wording of the description.
    const held = Boolean(row.metadata?.pricing_held);
    out.push({
      kind: "variation_applied", at,
      reference: (desc.split(" - ")[0] || "").trim(),
      amount: held ? 0 : money(desc),
      held,
    });
    return out;
  }

  if (row.action_type === "variation_approved") {
    out.push({
      kind: "variation_approved", at,
      reference: (desc.split(" - ")[0] || "").trim(),
    });
    return out;
  }

  if (row.action_type === "order_booking_updated") {
    // "Delivery changed from blank to 2026-09-12". The kind is on the metadata
    // rather than parsed back out of the sentence, because "Site measure" would
    // otherwise have to be matched by name.
    const m = desc.match(/changed from (.+?) to (.+?)$/i);
    const to = m?.[2]?.trim() || "";
    const from = m?.[1]?.trim() || "";
    out.push({
      kind: to.toLowerCase() === "cancelled" ? "booking_cancelled" : from.toLowerCase() === "blank" ? "booking_made" : "booking_moved",
      at,
      bookingKind: row.metadata?.kind || "delivery",
      from: from.toLowerCase() === "blank" ? null : from,
      to: to.toLowerCase() === "cancelled" ? null : to,
    });
    return out;
  }
  ...
  if (row.action_type === "order_item_updated") {
    ...
    // Whether we can say when this one lands yet. An ordered line with nothing
    // behind it says so out loud rather than leaving the customer to ask.
    const etaKnown = !!item?.supplier_eta;

    // THE ORDERED DATE IS READ FIRST, because it belongs to the status line
    // rather than beside it. Marking ten doors ordered sets the status AND the
    // date in one go, and "10 x Door ordered" followed by "ordered on 24
    // August" is the same fact twice.
    //
    // It is also the date the CUSTOMER cares about: when they were ordered, not
    // when somebody got round to ticking the box, which is all the activity
    // row's own timestamp knows.
    const orderedOn = (() => {
      const found = desc.match(/Supplier ordered changed from .+? to (.+?)(?:;|$)/i);
      const value = found?.[1]?.trim();
      return value && value.toLowerCase() !== "blank" ? value.slice(0, 10) : null;
    })();
    ...
    desc.split(";").forEach((part) => {
      ...
      if ((m = text.match(/^Status changed from (.+?) to (.+)$/i))) {
        out.push({
          kind: "item_status", at, from: m[1].trim(), to: m[2].trim(), qty, itemLabel: label,
          on: orderedOn || dayOf(at), etaKnown,
        });
      } else if ((m = text.match(/^Supplier ordered changed from (.+?) to (.+)$/i))) {
        // Only on its own. A date set alongside a status change is already in
        // the line above.
        const to = m[2].trim();
        if (!/Status changed from/i.test(desc) && to.toLowerCase() !== "blank") {
          out.push({ kind: "item_ordered", at, to: to.slice(0, 10), qty, itemLabel: label, etaKnown });
        }
...
function rollUp(changes) {
  ...
  changes.forEach((change) => {
    const rollable = change.kind === "item_status" || change.kind === "item_stage" || change.kind === "item_ordered";
    if (!rollable) {
      rolled.push(change);
      return;
    }
    ...
    if (seen) {
      ...
      // Ten doors ordered together are one line, and if a date is known for any
      // of them the line does not get to say the completion date is unknown.
      if (change.etaKnown) seen.etaKnown = true;
      return;
    }
    const copy = { ...change };
...
export async function loadCustomerUpdates(supabase, { from, to }) {
  ...
  const [ordersResult, itemsResult] = await Promise.all([
    ...
    //
    // supplier_eta is read as it stands TODAY, not as it stood when the item was
    // ordered, because it decides whether we tell the customer the completion
    // date is still to come. An ETA entered since is an answer, and saying TBC
    // after we have one would be wrong the moment it is sent.
    supabase.from("pcd_order_line_items").select("id, qty, title, product_type, supplier_eta").in("order_id", orderIds),
  ]);
