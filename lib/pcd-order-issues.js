...
export function issueKindLabel(key) { return labelFrom(ISSUE_KINDS, key, "Something else"); }
...
// refusing as a whole.
export function validateIssue(payload) {
  const errors = {};
  const kind = payload?.kind;
  const detail = String(payload?.detail || "").trim();

  if (!kind || !ISSUE_KIND_KEYS.includes(kind)) errors.kind = "Choose what is wrong.";
