...
export function lineDescription(item) {
  ...
  const parts = [what.toLowerCase() === "base_cabinet" ? "Base Cabinet" : what];

  if (toNumber(item.height_mm) > 0) parts.push(`Height: ${toNumber(item.height_mm)}mm`);
  if (toNumber(item.width_mm) > 0) parts.push(`Width: ${toNumber(item.width_mm)}mm`);
  ...
  if (edge) parts.push(`with ${edge} edges`);
  ...
  if (cups) {
    ...
    parts.push(`with ${cups} hinge holes${side ? ` hinged ${side.toLowerCase()}` : ""}`);
  }
