...
export const DEFAULT_BUSINESS_DEFAULTS = {
  ...
  hinge_supply_unit_cost_ex_gst: 15,
...
export function normalizeBusinessDefaults(defaults = {}) {
  return {
    ...
    hinge_supply_unit_cost_ex_gst: toNumber(
      defaults.hinge_supply_unit_cost_ex_gst ?? defaults.hingeSupplyUnitCostExGst,
      DEFAULT_BUSINESS_DEFAULTS.hinge_supply_unit_cost_ex_gst
    ),
...
export function calculateQuoteLine(line = {}, defaults = {}) {
  ...
  const hingeSupplyUnitCost = toNumber(
    line.hinge_supply_unit_cost_ex_gst ?? line.hingeSupplyUnitCostExGst,
    calculationDefaults.hinge_supply_unit_cost_ex_gst
  );
  const hingeDrillingCostExGst = line.hinge_holes || line.hingeHoles ? roundMoney(hingeQty * hingeDrillingUnitCost * qty) : 0;
  const hingeSupplyCostExGst = line.hinge_supply || line.hingeSupply ? roundMoney(hingeQty * hingeSupplyUnitCost * qty) : 0;
  ...
  const labourHours = toNumber(line.labour_hours ?? line.labourHours);
...
export function calculateQuoteTotals(lines = [], gstRate = GST_RATE, costs = {}) {
  ...
  const quoteLabourHours = toNumber(costs.labour_hours ?? costs.labourHours);
  const lineLabourHours = normalizedLines.reduce((sum, line) => sum + toNumber(line.labour_hours), 0);
  const labourHours = roundMoney(quoteLabourHours + lineLabourHours);
  const workerHourlyRate = toNumber(costs.worker_hourly_rate ?? costs.workerHourlyRate, calculationDefaults.worker_hourly_rate);
