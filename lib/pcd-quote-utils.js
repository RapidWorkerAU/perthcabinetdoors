...
export function calculateQuoteLine(line = {}, defaults = {}) {
  ...
  const processingLabourHours = !isCabinetLine && isDecorativeBoardLine(line)
    ...
    : 0;
  // Anything typed against the line by hand is kept and the processing time is
  // added to it, so wiring this up never silently discards an entered figure.
  const labourHours = isCabinetLine
    ? roundMoney(toNumber(calculationDefaults.labour_hours_per_cabinet) * qty)
    : roundMoney(toNumber(line.labour_hours ?? line.labourHours) + processingLabourHours);
