...
import { cupPositions, hingeCount, normaliseHingeSide, readMiddles } from "./pcd-hinges";
...
export const QUOTE_STATUSES = ["draft", "sent", "viewed", "approved", "rejected"];
...
export function formatItemSpecs(item, { includeQty = false } = {}) {
  ...
  return [size, finish, cabinet, drilling, qty].filter(Boolean).join(" | ");
}
...
export function hingeSpecText(item) {
  ...
  const cups = cupPositions(item);
  return [
    `Drill${count ? ` ${count}` : ""}`,
    side ? side.toLowerCase() : "side not recorded",
    cups ? `cups ${cups.join("/")}mm` : "standard cups",
  ].join(" ");
}
