...
export async function loadCustomerDesk(supabase, customerId) {
  ...
  const [
    ...
    supabase.from("pcd_tickets").select("*").eq("customer_id", customerId).order("last_message_at", { ascending: false }),
    supabase.from("pcd_messages").select(MESSAGE_FIELDS).eq("customer_id", customerId).order("created_at", { ascending: false }),
    supabase.from("pcd_agents").select("id,name,login_email"),
    supabase.from("pcd_pending_customer_changes").select("*").eq("customer_id", customerId).eq("status", "pending"),
    supabase.from("pcd_quotes").select("id,quote_number,title,status,total_inc_gst,created_at,order_id").eq("customer_id", customerId).order("created_at", { ascending: false }),
    supabase.from("pcd_orders").select("id,order_number,name,status,total_inc_gst,created_at").eq("customer_id", customerId).order("created_at", { ascending: false }),
    supabase.from("pcd_order_activity").select("*").eq("customer_id", customerId).order("created_at", { ascending: false }).limit(200),
    supabase.from("pcd_enquiries").select("id,topic,message,status,created_at").eq("customer_id", customerId),
    supabase.from("pcd_quote_requests").select("id,product_name,cabinet_brand,status,created_at").eq("customer_id", customerId),
  ]);
