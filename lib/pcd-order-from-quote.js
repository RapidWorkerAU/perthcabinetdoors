...
import { scheduleDate } from "./pcd-order-schedule";
import { spendCreditsOnOrder } from "./pcd-customer-credits";
...
/**
 * Take back an order row whose lines could not be written.
 *
 * WHY THIS EXISTS. Raising an order is several writes and there is no
 * transaction to put them in. The order row goes in first, and if the lines then
 * refuse, what is left behind is an order with nothing on it: invisible as a
 * problem, visible on the board as a job, and impossible for the customer to get
 * past, because the acceptance route has already marked their quote approved and
 * a second attempt is refused as already answered.
 *
 * An order with no lines is worth nothing to anybody, so it goes, and the error
 * travels on to whoever asked. The quote is then still approved, which is not
 * ideal, but it is a state somebody in the office can see and fix, rather than a
 * job that looks real and is not.
 *
 * Never throws. This runs while another error is already on its way up, and a
 * failure to tidy up must not replace the reason things went wrong.
 */
async function discardHalfMadeOrder(supabase, order) {
  if (!order?.id) return;
  try {
    await supabase.from("pcd_orders").delete().eq("id", order.id);
    console.error(
      `[order-from-quote] ${order.order_number}: the lines could not be written, so the empty order row was removed. ` +
        "The quote is still marked approved and needs looking at."
    );
  } catch (cleanupError) {
    console.error(
      `[order-from-quote] ${order.order_number}: the lines could not be written AND the empty order row could not be ` +
        "removed. It is on the board with nothing on it and needs deleting by hand.",
...
export async function createOrderFromQuote(supabase, quote, { actorType = "customer", markAcceptedAt = true } = {}) {
  ...
  const orderRow = {
      ...
      // THE DATES THE CUSTOMER WAS SHOWN BECOME THE JOB.
      //
      // The quote suggests a start and a completion, the customer accepts with
      // those dates in front of them, and this is where they stop being a
      // suggestion. From here the order owns them: the calendar draws the run
      // off this pair, the board counts down to the start, and changing them is
      // an order update rather than anything to do with the quote.
      //
      // COPIED EVEN WHEN THE START HAS ALREADY PASSED. A quote accepted weeks
      // late carries a start date that is now behind us, and the temptation is
      // to leave the order unscheduled instead. That is worse: an order with no
      // dates is invisible to the calendar and to every list that plans work,
      // so it is nobody's problem until a customer rings. A date somebody can
      // see and correct is the one that gets corrected. The order page says so.
      // See staleScheduleNotice.
      scheduled_start_date: scheduleDate(quote.suggested_start_date),
      target_completion_date: scheduleDate(quote.suggested_completion_date),
  ...
  // ── EVERYTHING THAT CAN REFUSE HAPPENS BEFORE ANYTHING IS WRITTEN ──────────
  //
  // These reads and the check below used to sit AFTER the order row was
  // inserted, and the check's own comment called itself the last chance. It was
  // the last chance far too late: the acceptance route marks the quote approved
  // before calling this, so a quote with an unconfigured cabinet left the
  // customer approved, with an order row carrying no lines, looking at an error,
  // and unable to try again because the quote now read as already responded to.
  // Nothing told the office either.
  //
  // Nothing here needs the order to exist, so there was never a reason for it to.
  ...
  // A cabinet nobody configured would reach the workshop as a sheet with no
  // panels to cut. Refused here, before an order exists, so the customer can go
  // back and the quote is still theirs to accept.
  ...
  if (lines?.length) {
    ...
    for (let attempt = 0; attempt <= CARRIED_SPEC_COLUMNS.length + 1; attempt += 1) {
      ...
      if (!column || dropped.includes(column)) {
        await discardHalfMadeOrder(supabase, order);
    ...
    if (!inserted) {
      await discardHalfMadeOrder(supabase, order);
      throw lastError || new Error("The order lines could not be saved.");
    }
    if (dropped.length) {
      console.error(
  ...
  // THE CREDIT STOPS BEING OURS TO MOVE HERE.
  //
  // Each credit the quote was holding becomes a paid payment row on the order,
  // one row per credit so the payments read back as what actually happened
  // rather than as one lump nobody can trace. From this point the balance owing
  // is right on every screen with no new arithmetic anywhere, because the
  // balance has always been the total less what came in.
  //
  // Never throws. An order the customer has accepted must exist whatever
  // happens to the bookkeeping, and a credit still sitting as held is visible
  // on the customer record rather than lost.
  try {
    await spendCreditsOnOrder(supabase, { quoteId: quote.id, orderId: order.id });
  } catch (creditError) {
    console.error(
      `[order-from-quote] ${order.order_number}: the customer's credit did not reach the order:`,
