...
import { releaseCreditsForQuote } from "./pcd-customer-credits";
...
export async function runQuoteExpirySweep(supabase, { now = new Date(), baseUrl = "", validDays } = {}) {
  ...
  for (const quote of quotes || []) {
    ...
    try {
      ...
      // A QUOTE THAT EXPIRED LETS GO OF THE CUSTOMER MONEY IT WAS HOLDING.
      // Only after the claim above succeeded, so a quote approved in the last
      // few seconds keeps its credit. The money stays the customer's either
      // way; this only frees it for the next quote we write them.
