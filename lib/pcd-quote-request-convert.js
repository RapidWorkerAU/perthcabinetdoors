...
import { bandedEdgeList, edgeFinishFromBanded } from "./pcd-line-details";
...
import { hardwareLinePatch } from "./pcd-hardware-line";
...
/**
 * What a quote made from a request is called: the customer and the suburb.
 * Null when neither is known, so the quote falls back to its own title.
 */
export function projectNameFromRequest(quoteRequest = {}) {
  const parts = [quoteRequest?.customer_name, quoteRequest?.delivery_suburb]
...
export function convertedQuoteLine(line, { resolveBoard, resolveHardware = null, quoteRequest, businessDefaults }) {
  const notes = withHingeSupplyNote(line.notes, line);
  const base = {
    product_type: line.product_type,
    // Carried, not derived. The items table reads panel_use to show a Scribe as
    // a Scribe, and a conversion that drops it puts the customer's answer in
    // the bin between the website and the bench.
    panel_use: line.panel_use || null,
    // Carried exactly, and edge_finish worked out from the exact answer so
    // every screen, PDF and order form still reading the old three phrase field
    // gets a true one without anybody editing them.
    banded_edges: Array.isArray(line.banded_edges) ? bandedEdgeList(line.banded_edges) : null,
    edge_finish: Array.isArray(line.banded_edges)
      ? edgeFinishFromBanded(line.banded_edges)
      : line.edge_finish || null,
    hole_type: line.hole_type || null,
  ...
  // HARDWARE, PRICED FROM THE CATALOGUE ROW THE CUSTOMER PICKED. It used to
  // skip pricing altogether, so a hinge from the website landed at no price
  // and, being skipped, never appeared on the list of lines with no price
  // either. Filled the same way as picking it in the quote editor.
  if (line.product_type === "Hardware" && resolveHardware) {
    const item = resolveHardware({ id: line.hardware_catalogue_id, name: line.product_name });
    if (item) {
      const patch = hardwareLinePatch(item);
      return {
        line: { ...base, ...patch, qty: base.qty, notes: base.notes },
        match: { ok: true },
        skipped: false,
      };
    }
    return {
      ...
      match: {
        ok: false,
        reason: "hardware_not_found",
        message: "This hardware is not in the catalogue under that name any more, so it has no price. Pick it again on the quote.",
      },
      skipped: false,
    };
  }
