...
//  - panel_to_floor (base/tall/blind corner) drops the bottom to the floor,
//    closing the kickboard recess on that side;
//  - panel_to_ceiling raises the top to the room ceiling — a full-height
//    finished end/side, instead of stopping at the cabinet top.
// panel_to_floor and panel_to_ceiling set the bottom and top separately; with
// neither set the panel stays at the carcass height, exactly as before.
export function finishPanelVerticalSpanMm(item, roomHeightMm) {
  ...
  const bottomMm = isWall
    ...
    : (item?.panel_to_floor ? 0 : spanBottom);
  const ceiling = Number(roomHeightMm);
  const topMm = (item?.panel_to_ceiling && ceiling > 0) ? ceiling : spanTop;
