...
export function dismissReason(key) {
  return DISMISS_REASONS.filter((r) => r.key === key)[0] || DISMISS_REASONS[DISMISS_REASONS.length - 1];
}

export function dismissReasonLabel(key) {
  return dismissReason(key).label;
}

export function isDismissReason(key) {
  return DISMISS_REASON_KEYS.includes(key);
}
 ...
 */
export function dismissNote(cardLabel, reasonKey, detail) {
  const reason = dismissReason(reasonKey);
  const extra = String(detail || "").trim();
  const what = String(cardLabel || "A board card").trim();
  return `${what} set aside. ${reason.words}${extra ? ` ${extra}` : ""}`;
}

export function validateDismissal(payload) {
  const errors = {};
  if (!isDismissReason(payload?.reason)) errors.reason = "Choose why it is being set aside.";
