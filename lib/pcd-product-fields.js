 ...
 * @property {boolean} [internal] staff only, never offered to a customer
 */
...
export const PRODUCT_FIELDS = {
  ...
  // A SHEET OF LAMINATE, SOLD AS A SHEET. Not a front, so nothing is routed
  // into it, no edge is taped and no cup is bored; it has a size because we cut
  // sheets down, and a material and colour because it comes out of the colour
  // library like every other board.
  //
  // internal: true keeps it off the public request form. Ashleigh's call, and
  // the reason is worth keeping: it sits right beside decorative board on the
  // list, it is cheaper, and a customer who picks it when they meant a door is
  // a correction we would be making on every order forever. Staff can pick it
  // on a quote line and on the order form we send out; the website cannot.
  Laminate: {
    label: "Laminate",
    blurb: "A sheet of 0.7mm laminate, sold as the sheet.",
    board: true,
    hardware: false,
    size: true,
    profile: false,
 ...
 *
 * Two things are dropped: a type with no record here at all (it would fall
 * through to the permissive fallback and ask a customer about a profile on a
 * carcass), and a type marked internal. The internal filter is the one seam
 * that keeps a staff-only type out of the public form, so a new one is added by
 * setting the flag above rather than by writing a second list of types that
 * would then have to be kept in step with this one.
 */
export function productTypeChoices(types = []) {
  return types
    .filter((type) => PRODUCT_FIELDS[type] && !PRODUCT_FIELDS[type].internal)
    .map((type) => ({ value: type, ...PRODUCT_FIELDS[type] }));
}

/** Is this type staff only, kept off every customer-facing screen? */
export function isInternalType(productType) {
  return fieldsForProductType(productType).internal === true;
}
