...
import { quoteShell } from "./pcd-email-templates";
import { termsHtmlForEmail } from "./pcd-terms-html";
 ...
 * ── NO MONEY ON A REPLY ──────────────────────────────────────────────────────
 *
 * This used to print "Total inc GST" under the reference. It is deliberately
 * gone. A reply is a conversation, and a figure stapled to the bottom of one
 * turns every exchange about a delivery date into an exchange about a price. It
 * is also a number we then have to keep true: a variation approved on Tuesday
 * makes Monday's reply wrong, and nothing goes back to correct an email.
 *
 * The quote and the invoice are where a figure belongs, because those are the
 * documents that carry a date and a version.
 */
...
export function deskReplyEmailHtml({ bodyHtml, signatureHtml = "", reference = null, subject = "" }) {
  // Spaced the way it was typed. Without this the mail client's own paragraph
  // margins take over and one blank line becomes four. See termsHtmlForEmail.
  const body = termsHtmlForEmail(bodyHtml);
  const signature = termsHtmlForEmail(String(signatureHtml || "").trim());

  // ON THE CREAM SHELL, like every other email a customer receives. It was on
  // the navy staff shell, which meant a reply from us looked nothing like the
  // quote it was about.
  return quoteShell({
    ...
    footerNote: "You can reply to this email and it comes straight back to our team.",
    children: `
      <div style="color:#0f172a;font-size:15px;line-height:23px;">${body}</div>
