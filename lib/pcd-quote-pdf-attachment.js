...
import { applyCredits, creditsHeldByQuote } from "./pcd-customer-credits";
...
export async function loadQuoteForPdf(supabase, quoteId) {
  ...
  // THE CREDIT'S WORDS, FOR THE PDF.
  //
  // The figure is already on the quote row, but the label is not: it names what
  // they paid for and when, which is what somebody checking the PDF against a
  // bank statement needs. Read here so the PDF says exactly what the web page
  // says. Never throws: a quote must still print if the credits cannot be read.
  let creditLabelForPdf = "";
  try {
    const held = await creditsHeldByQuote(supabase, quoteId);
    const { lines: creditLines } = applyCredits(held, quote.total_inc_gst);
    creditLabelForPdf = creditLines.length === 1
      ? creditLines[0].label
      : creditLines.length > 1
        ? `Already paid, ${creditLines.length} credits`
        : "";
  } catch (creditError) {
    console.error("[quote-pdf] could not read the credits for this quote:", creditError?.message || creditError);
  }

  return {
    quote: { ...quote, credit_label: creditLabelForPdf },
...
// A quote can be sent more than once: a price is corrected, a line is added, or
// an admin override pulls it back to edit it.
//
// This USED TO DELETE the previous PDF and its file. That kept the customer's
// list clean, which was the point, but it meant exactly one PDF ever existed and
// there was no way to show what they had originally been given. With the
// override making a pull-back a normal action, every one of those destroyed the
// record of what had already gone out.
//
// So the old copy is MARKED rather than removed. It stays in storage and in the
// table, the customer's list hides it, and the office can still open it to
// answer "what did they actually receive in July".
//
...
async function supersedePreviousQuotePdf(supabase, quoteId) {
  ...
  if (error) {
    // The column is not there yet, so fall back to the old behaviour rather
    // than failing a send. A quote that cannot reach the customer is worse than
    // one whose previous PDF was not kept.
    if (!isMissingSupersededColumn(error)) throw error;
    return removePreviousQuotePdfLegacy(supabase, quoteId);
  }

  const ids = (data || []).map((attachment) => attachment.id).filter(Boolean);
...
function isMissingSupersededColumn(error) {
  return error?.code === "PGRST204" || String(error?.message || "").includes("superseded_at");
}

// Only reached on a database that has not had 202608221000 run yet.
async function removePreviousQuotePdfLegacy(supabase, quoteId) {
  const { data, error } = await supabase
    .from("pcd_quote_attachments")
    .select("id,file_path,file_name")
    .eq("quote_id", quoteId)
  ...
  console.error(
    "[quote-pdf] pcd_quote_attachments.superseded_at is missing, so the previous quote PDF was deleted " +
      "rather than kept. Run supabase/202608221000_pcd_quote_attachment_superseded.sql to start keeping " +
      "a record of what each customer was sent."
  );
}
...
export async function attachQuotePdf(supabase, quoteId, { includeCabinetDrawings = false } = {}) {
  ...
  await supersedePreviousQuotePdf(supabase, quoteId);
