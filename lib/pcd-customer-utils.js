import { ADDRESS_KEYS, formatSiteAddress } from "./pcd-contact-details";
...
/**
 * The address columns, with the one liner rebuilt from the parts.
 *
 * THE ONE LINER FOLLOWS THE PARTS, and this is not a nicety. site_address is
 * what every list, label and PDF actually reads; the three parts are what the
 * edit screens actually write. A screen that sent only the parts left
 * site_address holding the OLD address, so a customer moved to Myaree still
 * read as their previous suburb in the customers table, on their orders, and on
 * the delivery label printed from one.
 *
 * Rebuilt here rather than in each screen, because the screen that forgets is
 * the one nobody notices until a van goes to the wrong house.
 *
 * A payload carrying NO parts at all is a form that only ever had one box, and
 * its own one liner is then the best answer there is. That is why this looks at
 * whether the keys were sent rather than at whether they have values: clearing
 * the three boxes has to be able to clear the address too.
 */
export function addressColumnsFromPayload(payload = {}) {
  const sent = ["site_street", "site_suburb", "site_postcode"].some(
    (key) => payload[key] !== undefined
  );

  const street = cleanText(payload.site_street);
  const suburb = cleanText(payload.site_suburb);
  const postcode = cleanText(payload.site_postcode);

  return {
    site_address: sent
      ? cleanText(formatSiteAddress({ street, suburb, postcode }))
      : cleanText(payload.site_address ?? payload.address),
    site_street: street,
    site_suburb: suburb,
...
export function normalizeCustomerPayload(payload = {}, options = {}) {
  ...
  return {
    ...
    email,
    phone,
    ...addressColumnsFromPayload(payload),
