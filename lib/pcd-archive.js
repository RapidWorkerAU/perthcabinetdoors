...
// WHY A REASON, when archived already means "stop showing me this".
//
// Because two very different things now put a quote here. A person filing away
// a job that was quoted twice is a fact about the list. A quote the customer
// went quiet on for thirty days is a lost lead, and lead conversion has to keep
// counting it as one or the rate it reports climbs back toward 100%, which is
// the exact thing that report exists to stop. See lib/pcd-quote-expiry.js.
//
// Orders have no equivalent and do not carry this. Only quotes expire.
export const ARCHIVED_MANUAL = "manual";
export const ARCHIVED_EXPIRED = "expired";
...
/**
 * The same, for a quote, which also records WHY.
 *
 * Separate from archivePatch rather than an argument on it, because orders go
 * through the same helper and have no reason to carry a column they do not have.
 */
export function quoteArchivePatch(row, reason = ARCHIVED_MANUAL, now = new Date().toISOString()) {
  return { ...archivePatch(row, now), archived_reason: reason };
}
...
/**
 * Restoring a quote, which also forgets why it was archived.
 *
 * It has to be cleared, not left. A restored quote is live again and its clock
 * restarts from whenever it is next sent; a stale 'expired' sitting on it would
 * keep it in the lapsed column of a report that is meant to be about quotes
 * nobody answered.
 */
export function quoteRestorePatch(row, fallbackStatus) {
  return { ...restorePatch(row, fallbackStatus), archived_reason: null };
}
