...
import { lineBandedEdges, panelBoardAnswers, panelDef, panelProfile } from "./pcd-panel-options";
...
export function finishPanelBoard(item, overrideKey = null, panelKey = null) {
  ...
  const base = {
    ...
    // What kind of panel it is and which of its edges are taped, so the line
    // arrives on the quote as a Filler with its edges rather than a bare Panel.
    ...panelBoardAnswers(item, panelKey),
  };
...
export function panelBoardFields(board) {
  return {
    ...
    // The kind of panel, and its banded edges on decorative board only. The
    // override above can change the material, so this is decided on the board
    // the panel is actually cut from.
    panel_use: board.panelUse || "",
    banded_edges: board.panelKey ? lineBandedEdges(board.material, board.bandedEdges) : null,
...
/**
 * The board a finishing panel is cut from, by the same route its quote line
 * takes, so the design tool offers banded edges on exactly the panels that
 * reach the quote as decorative board. A side filler takes the door board, the
 * same as sideFillerLinesForCabinet.
 */
export function panelBoardMaterial(item, panelKey) {
  const def = panelDef(panelKey);
  if (!def || !item) return "";
  if (panelKey === "side_filler_left" || panelKey === "side_filler_right") {
    return item.door_style?.material || item.material || "";
  }
  if (def.board === "carcass") return carcassPanelBoard(item, def.styleKey, null, panelKey).material || "";
  return finishPanelBoard(item, def.styleKey, panelKey).material || "";
}
