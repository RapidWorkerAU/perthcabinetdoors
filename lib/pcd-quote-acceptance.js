 ...
 *
 * ── THE CREDIT COMES OFF THE DEPOSIT ─────────────────────────────────────────
 *
 * A customer holding a credit has already paid us that money, so there is that
 * much less to pay to start. Taken off here rather than at each call site
 * because four things ask this question (the payment gate, the deposit
 * reminders, the public quote page and the acceptance itself) and all four have
 * to get the same answer.
 *
 * The percentage is still worked out on the FULL total: the deposit is a share
 * of the job, and the credit is money already received against it, not a
 * discount on the job's size. Never below zero, so a credit larger than the
 * deposit clears it and the rest lands on the balance.
 */
export function depositAmountForQuote(quote) {
  ...
  const full = Number(((total * percent) / 100).toFixed(2));
  const credit = Number(quote.credit_applied_inc_gst || 0);
  if (!Number.isFinite(credit) || credit <= 0) return full;
  return Math.max(0, Number((full - credit).toFixed(2)));
}

/** The deposit before any credit came off it. Shown beside the reduced one. */
export function depositBeforeCreditForQuote(quote) {
  if (!quote?.deposit_required) return 0;
  const percent = Number(quote.deposit_percent || 0);
  const total = Number(quote.total_inc_gst || 0);
  if (!Number.isFinite(percent) || percent <= 0 || !Number.isFinite(total) || total <= 0) return 0;
