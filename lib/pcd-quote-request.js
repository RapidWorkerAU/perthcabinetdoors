...
import { HOLE_TYPES, bandedEdgeList } from "./pcd-line-details";
...
// Same idea as the colour source columns above, for the cabinet the customer
// drew. Until the migration is run there is nowhere to put the box, so the
// lead is saved without it rather than refused: the request still reads
// correctly, and its cabinets convert the way they used to, needing the sizes
// typed in by hand.
function isMissingHardwareIdSchemaError(error) {
  const message = String(error?.message || "");
  return error?.code === "PGRST204" && message.includes("hardware_catalogue_id");
}

function withoutHardwareIdColumn(row) {
  const { hardware_catalogue_id: _id, ...rest } = row;
  return rest;
}

function isMissingPanelUseSchemaError(error) {
  const message = String(error?.message || "");
  return error?.code === "PGRST204" && message.includes("panel_use");
}

function withoutPanelUseColumn(row) {
  const { panel_use: _use, ...rest } = row;
  return rest;
}

function isMissingEdgeOrHoleSchemaError(error) {
  const message = String(error?.message || "");
  return error?.code === "PGRST204" && (message.includes("banded_edges") || message.includes("hole_type"));
}

function withoutEdgeAndHoleColumns(row) {
  const { banded_edges: _edges, hole_type: _hole, ...rest } = row;
  return rest;
}

function isMissingCabinetSpecSchemaError(error) {
  const message = String(error?.message || "");
  return error?.code === "PGRST204" && (message.includes("cabinet_spec") || message.includes("design_item_id"));
}

function withoutCabinetSpecColumns(row) {
  const { cabinet_spec: _spec, design_item_id: _itemId, ...rest } = row;
  return rest;
}
...
export function quoteRequestLineRow(line, index, { quoteRequestId = null, productName = null } = {}) {
  return {
    ...
    // WHICH KIND OF PANEL. A panel use is not a product type: the line is a
    // Panel and this says what sort. Without it every panel a customer sends
    // arrives on the quote as a plain Panel and somebody has to work out from
    // the notes whether it is a scribe, a filler or a kickboard.
    panel_use: line.panelUse || null,
    // WHICH EDGES GET TAPE, and WHICH BORING. Both are decisions the workshop
    // acts on and neither had anywhere to live, so both used to travel as a
    // sentence in the notes or not at all.
    banded_edges: Array.isArray(line.bandedEdges) ? bandedEdgeList(line.bandedEdges) : null,
    hole_type: HOLE_TYPES.includes(line.holeType) ? line.holeType : null,
    ...
    // And WHICH catalogue row, so the conversion can price it. This used to be
    // left off on purpose, on the reasoning that the name was enough to find it
    // again; but the website names a row by brand and name and the quote editor
    // adds the SKU, so nothing ever found it, and every hinge from the website
    // converted at no price.
    hardware_catalogue_id: line.hardwareCatalogueId || null,
    ...
    // THE CABINET ITSELF, on a base_cabinet line built from a design.
    //
    // A cabinet is priced from a cut list, and a cut list needs the box: its
    // height, width and depth, the carcass thickness, whether there is a back,
    // how many shelves and where they sit. None of that had anywhere to live on
    // a request line, so it travelled as a sentence in the notes and the
    // conversion had nothing to build a cabinet from. See
    // lib/pcd-cabinet-from-design.js.
    design_item_id: line.designItemId || null,
    cabinet_spec: line.cabinetSpec || null,
...
export async function insertQuoteRequest(supabase, payload) {
  ...
  // Set when the cabinet columns turn out not to exist. The lead is still
  // saved; its cabinets just arrive on the quote unmeasured, as they used to.
  let cabinetSpecDropped = false;
  if (lines.length) {
    let rows = lines.map((line, index) =>
      quoteRequestLineRow(line, index, { quoteRequestId: requestRow.id, productName: payload.productName })
    );
    let { error: linesError } = await supabase.from("pcd_quote_request_line_items").insert(rows);
    // The newest late column, so tried first. Without it the request still
    // saves; its hardware is matched back by name at conversion instead.
    if (linesError && isMissingHardwareIdSchemaError(linesError)) {
      rows = rows.map(withoutHardwareIdColumn);
      ({ error: linesError } = await supabase.from("pcd_quote_request_line_items").insert(rows));
      console.error(
        "[quote-request] hardware_catalogue_id is missing from pcd_quote_request_line_items. Saved without it. " +
          "Run supabase/202609111200_pcd_line_answers_reach_the_order.sql: until then website hardware is matched " +
          "back to the catalogue by name when the request is converted."
      );
    }
    // A live site must not start rejecting quote requests because a migration
    // has not been run yet. If the cabinet columns are not there, drop them and
    // save the lead: every other field still lands, and a converted cabinet
    // needs its sizes typed in the way it always did.
    // A panel use is the newest of the late columns, so it is tried first: a
    // site that has not run the migration yet still takes the request, it just
    // cannot say which kind of panel until somebody does.
    if (linesError && isMissingPanelUseSchemaError(linesError)) {
      rows = rows.map(withoutPanelUseColumn);
      ({ error: linesError } = await supabase.from("pcd_quote_request_line_items").insert(rows));
      console.error(
        "[quote-request] panel_use is missing from pcd_quote_request_line_items. Saved without it. " +
          "Run supabase/202609082000_pcd_quote_request_panel_use.sql: until then every panel from the " +
          "website converts as a plain Panel and the kind has to be read out of the notes."
      );
    }
    if (linesError && isMissingEdgeOrHoleSchemaError(linesError)) {
      rows = rows.map(withoutEdgeAndHoleColumns);
      ({ error: linesError } = await supabase.from("pcd_quote_request_line_items").insert(rows));
      console.error(
        "[quote-request] banded_edges / hole_type are missing from pcd_quote_request_line_items. Saved " +
          "without them. Run supabase/202609082100_pcd_banded_edges_and_hole_type.sql: until then which " +
          "edges are taped and which hinge boring is wanted have to be read out of the notes."
      );
    }
    if (linesError && isMissingCabinetSpecSchemaError(linesError)) {
      rows = rows.map(withoutCabinetSpecColumns);
      ({ error: linesError } = await supabase.from("pcd_quote_request_line_items").insert(rows));
      cabinetSpecDropped = true;
      console.error(
        "[quote-request] cabinet_spec / design_item_id are missing from pcd_quote_request_line_items. " +
          "Saved without them. Run supabase/202609011200_pcd_quote_request_cabinet_spec.sql: until then a " +
          "cabinet from a customer's design converts with no size, shelves or cut list on it."
      );
    }
    // If the two colour-source columns are not there, drop them too: the spec
    // still lands, the conversion just falls back to matching the colour by name.
    if (linesError) {
  ...
  await logOrderActivity(supabase, {
    ...
    metadata: {
      ...
      ...(colourSourceDropped
        ...
        : {}),
      ...(cabinetSpecDropped
        ? { cabinet_spec_column_missing: true, cabinet_note: "Saved without the cabinet box. Run the cabinet spec migration; until then a cabinet converts with no size, shelves or cut list." }
        : {}),
    },
...
export async function sendQuoteRequestEmails(payload) {
  ...
  // THE LINES AS WE STORED THEM, not as the form sent them. The customer's
  // copy is their check that we got it right, so it shows what we saved, after
  // the same rules the database row went through: an edge a thermolaminate
  // front cannot take is not shown, and a door that is not drilled has no cups.
  const lines = (Array.isArray(payload.lines) ? payload.lines : []).map((line, index) =>
    quoteRequestLineRow(line, index, { productName: payload.productName })
  );
  const shown = { ...payload, lines };
  ...
  results.push(await attemptSend("business", () => resend.emails.send({
    ...
    html: businessQuoteRequestHtml(shown),
    text: [
  ...
  if (payload.customerEmail) {
    results.push(await attemptSend("customer", () => resend.emails.send({
      ...
      html: customerQuoteRequestHtml(shown),
      text: [
