...
import { computeFillerPanelRun, fillerPanelSegment, fillerPanelGapMm, sideFillerWidthMm } from "./pcd-fillerpanel-utils";
...
import { resolveRunnerHardware, runnerNoteLabel } from "./pcd-drawer-utils";
import { materialLabelForType } from "./pcd-colour-library";
import { finishPanelVerticalSpanMm } from "./pcd-finishpanel-utils";
import { itemBandedEdges, lineBandedEdges, panelBoardAnswers, panelNeedsKickboard, panelProfile, panelReach } from "./pcd-panel-options";
...
import { accessoryHeightMm, accessoryLabel, accessoryProblems, insideWidthMm, readAccessories } from "./pcd-cabinet-accessories";
...
// THE FRONTS' EDGES AND BORING. Doors and drawer fronts on one cabinet share
// one banded edges setting (all four until changed), taped only when the front
// is decorative board; and a drilled door carries the cabinet's hinge boring.
// These used to reach the quote as nothing at all.
function frontAnswers(item, style, drilled = false) {
  return {
    banded_edges: lineBandedEdges(style?.material, itemBandedEdges(item)),
    hole_type: drilled ? item.hole_type || "" : "",
  };
}
...
function designItemToLine(item) {
  ...
  if (isCabinet) {
    ...
    // A standalone door carries its own boring; a scribe says it is a scribe;
    // and every decorative board piece carries its own banded edges.
    line.hole_type = item.hinge_holes ? item.hole_type || "" : "";
    line.panel_use = item.item_type === "scribe" ? "Scribe" : "";
    line.banded_edges = lineBandedEdges(item.material, itemBandedEdges(item));
...
function doorLinesForCabinet(item, roomName, { cabinetIncluded = true } = {}) {
  ...
  return sizes.map((size) => {
    ...
    return {
      ...
      ...frontAnswers(item, style, size.hingeQty > 0),
...
function cornerDoorLinesForCabinet(item, roomName, { cabinetIncluded = true } = {}) {
  ...
  return computeCornerDoorLeaves(item).map((leaf) => {
    ...
    return {
      ...
      ...frontAnswers(item, style, leaf.isHingeLeaf && leaf.hingeQty > 0),
    };
  });
}
...
function drawerLinesForCabinet(item, roomName, { cabinetIncluded = true } = {}) {
  ...
  // Names the real runner where one has been picked, and falls back to the
  // generic spec for every drawer designed before runners were a library
  // item. See runnerNoteLabel.
  const runnerNote = `Runner: ${runnerNoteLabel(cfg)}.`;
...
function mixedLinesForCabinet(item, roomName, { cabinetIncluded = true, includeDoors = true, includeDrawers = true } = {}) {
  ...
  sections.forEach((sec, idx) => {
    ...
    if (sec.type === "drawers") {
      ...
      // Same rule as the plain drawer bank above: name the real runner where
      // one is picked, fall back to the generic spec where it is not.
      const runnerNote = `Runner: ${runnerNoteLabel(cfg)}.`;
      ...
      computeDoorSizesForConfig(cfg, widthMm, heightMm).forEach((size) => {
        ...
        lines.push({
          ...
          ...frontAnswers(item, doorStyle, size.hingeQty > 0),
...
function sideFillerLinesForCabinet(item, roomName, room, allRoomItems = []) {
  ...
  const push = (name, widthMm, panelKey) => {
    ...
    lines.push({
      ...
      qty: perCabinetQty(item),
      ...panelBoardFields({ ...board, ...panelProfile(item, panelKey), ...panelBoardAnswers(item, panelKey) }),
    });
  };

  // Width falls back to the measured gap beside the cabinet, the same value the
  // plan, elevation and 3D draw it at, so the quote can never disagree with the
  // drawing about how wide a filler is.
  if (item.side_filler_left)  push("Side Filler (Left)",  sideFillerWidthMm(item, room, allRoomItems, "left"), "side_filler_left");
  if (item.side_filler_right) push("Side Filler (Right)", sideFillerWidthMm(item, room, allRoomItems, "right"), "side_filler_right");
  return lines;
}
...
// WHAT IS FITTED INSIDE THE CABINET: RAILS, BINS AND LIGHTS.
//
// One line per accessory, the same shape as the handle and the hinge above,
// because that is what they are: a bought item with a quantity. The quantity is
// how many are in ONE cabinet times how many of the cabinet there are.
//
// THE PRICE IS NOT SET HERE. The line carries the hardware row it was picked
// from (unit_cost_source_id) and no cost, and the import reads today's price
// out of the library against that id, the same way a board line is re-rated
// from the colour library. A rail that went up $2 last week is quoted at what
// it costs this week, and nobody has to re-pick it on every design.
//
// The line says where it goes and how wide the inside of the cabinet is, so
// whoever cuts a rail to length has the measurement without opening the design.
function accessoryLinesForCabinet(item, roomName) {
  const list = readAccessories(item);
  if (!list.length) return [];
  const traceLabel = [itemLabel(item), roomName].filter(Boolean).join(" — ");
  const inside = Math.round(insideWidthMm(item));
  return list.map((entry) => {
    const name = accessoryLabel(entry);
    const height = Math.round(accessoryHeightMm(entry, item));
    return {
      product_type: "Hardware",
      product_name: name,
      hardware_type: entry.type,
      description: traceLabel ? `${name} — ${traceLabel}` : name,
      notes: [
        `Fitted inside the cabinet at ${height}mm from the bottom.`,
        inside > 0 ? `Inside width ${inside}mm.` : "",
      ].filter(Boolean).join(" "),
      qty: entry.qty * (item.qty || 1),
      // Priced from the library at import. See withLibraryHardwareRates.
      unit_cost_source_id: entry.hardware_id,
      unit_cost_source_label: name,
...
// THE DRAWER RUNNERS, AS THEIR OWN LINES.
//
// A runner used to be three words on the drawer line and nothing else: no
// price, no line, no quantity. A bank of four drawers reached a quote with the
// runners invisible, and somebody either added them by hand or did not.
//
// Now a design can name the actual runner out of the hardware library, and
// when it does it becomes a line of its own, exactly as a hinge or a pull-out
// bin does. A design that has not named one still says its generic spec on the
// drawer line and produces no line here, which is what every drawer designed
// before this does.
//
// ── THE QUANTITY IS COUNTED, NOT TYPED ─────────────────────────────────────
//
// One runner per drawer, times how many of the cabinet there are. The count
// comes from frontCountsForHardware, the same function the handles use, so a
// drawer added to a bank changes the runner quantity with it and cannot be
// left behind. A mixed cabinet counts only the drawers in its drawer sections.
//
// Most runners are sold as a PAIR, and the library row says so in its name
// ("450NL 30kg Pair"). So one per drawer is one pair per drawer, which is
// what a drawer needs. A row priced per side would need two, and the honest
// place to say that is the library row itself rather than a guess here.
//
// THE PRICE IS NOT SET HERE, for the same reason as the accessories above:
// the line carries the library row it was picked from and the import reads
// today's price against that id. A runner that went up last week is quoted at
// this week's cost without anybody re-picking it.
function runnerLinesForCabinet(item, roomName) {
  const picked = runnerPicksForCabinet(item);
  if (!picked.length) return [];
  const traceLabel = [itemLabel(item), roomName].filter(Boolean).join(" — ");

  return picked.map(({ runner, drawers }) => ({
    product_type: "Hardware",
    product_name: runner.name || "Drawer runner",
    hardware_type: "drawer_runner",
    description: traceLabel ? `${runner.name || "Drawer runner"} — ${traceLabel}` : runner.name || "Drawer runner",
    notes: `One per drawer, for ${drawers} drawer${drawers === 1 ? "" : "s"}.`,
    qty: drawers * (item.qty || 1),
    // Priced from the library at import. See withLibraryHardwareRates.
    unit_cost_source_id: runner.id,
    unit_cost_source_label: runner.name || "Drawer runner",
    product_unit_cost_ex_gst: 0,
    unit_cost_mode: "manual",
  }));
}

// Which runners this cabinet needs and how many drawers each covers.
//
// A plain drawer bank has one answer. A MIXED cabinet can have several drawer
// sections, and each carries its own drawer config, so each can name a
// different runner: a deep pan drawer section on heavy runners above a bank of
// shallow ones is a real cabinet. Sections naming the same runner are added
// together rather than emitted twice.
function runnerPicksForCabinet(item) {
  const configs = [];
  if (item.front_type === "drawers") {
    const heights = item.drawer_config?.heights_mm;
    configs.push([item.drawer_config || {}, Array.isArray(heights) && heights.length ? heights.length : 1]);
  } else if (item.front_type === "mixed") {
    const sections = Array.isArray(item.section_config?.sections) ? item.section_config.sections : [];
    for (const section of sections) {
      if (section.type !== "drawers") continue;
      const heights = section.drawer?.heights_mm;
      configs.push([section.drawer || {}, Array.isArray(heights) && heights.length ? heights.length : 1]);
    }
  }

  const byRunner = new Map();
  for (const [cfg, drawers] of configs) {
    const runner = resolveRunnerHardware(cfg);
    if (!runner || drawers <= 0) continue;
    const held = byRunner.get(runner.id);
    if (held) held.drawers += drawers;
    else byRunner.set(runner.id, { runner, drawers });
  }
...
function selectionForItem(item, selections) {
  ...
  if (CABINET_TYPES.includes(item.item_type)) {
    return {
      ...
      // Rails, bins and lights fitted inside the cabinet. On unless somebody
      // unticks them, the same as every other part.
      accessories: sel.accessories !== false,
    };
  }
  ...
  for (const item of importableItems) {
    ...
    if (isCabinet) {
      ...
      if (sel.panels) {
        ...
        add("panels", sideFillerLinesForCabinet(item, roomName, room, items));
      ...
      // Runners ride with the drawers they carry, so unticking drawers drops
      // them the same way unticking doors drops the hinges.
      if (sel.drawers) add("drawers", runnerLinesForCabinet(item, roomName));
      // Rails, bins and lights fitted inside. Their own toggle, because they
      // are their own decision: a wardrobe's rail is wanted whether or not the
      // doors are being quoted with it.
      if (sel.accessories) add("accessories", accessoryLinesForCabinet(item, roomName));
    } else if (sel.include) {
      if (item.item_type === "floating_shelf") {
...
function computeItemWarnings({ importableItems, selections, selectedCabinetItems, roomNameById, roomById, isMadeToOrderBoard = () => false }) {
  ...
  for (const item of importableItems) {
    ...
    if (isCabinet) {
      ...
      // A blank width is no longer a hole: it falls back to the measured gap
      // beside the cabinet, which is what the drawing shows. So the warning is
      // now only for the case that really would be skipped, where nothing is
      // measurable there either. Measured against the importable items rather
      // than the whole room, which can only ever read a gap as WIDER than the
      // drawing does, so this never warns about a filler that will be fine.
      const sideFillerGapFor = (side) =>
        sideFillerWidthMm(item, roomById.get(item.room_id), importableItems, side);
      if (sel.panels && ((item.side_filler_left && !(sideFillerGapFor("left") > 0)) ||
                         (item.side_filler_right && !(sideFillerGapFor("right") > 0)))) {
        warnings.push({
          itemId: item.id,
          label: `${traceLabel} — a side filler is on but its gap width isn't set and there is nothing beside the cabinet to measure, so it would be skipped. Enter the side filler width or turn it off.`,
        });
      }
      // An accessory sitting above the top or below the floor of its cabinet.
      // Seen here rather than on the bench: a rail is cheap to move on a
      // drawing and expensive to move once it is drilled.
      if (sel.accessories) {
        for (const problem of accessoryProblems(item)) {
          warnings.push({ itemId: item.id, label: `${traceLabel} — ${problem} Check its height in Accessories.` });
        }
      }
...
export {
  ...
  accessoryLinesForCabinet,
  runnerLinesForCabinet,
