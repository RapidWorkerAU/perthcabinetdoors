...
// Re-exporting does NOT create a local binding, so the number has to be
// imported as well as passed through. The booking ask prints it.
import { BUSINESS_PHONE, SALES_EMAIL } from "./pcd-business-identity";
// The line describers moved to lib/pcd-line-summary.js the day the customer
// desk needed the same words about the same request. One definition, and an
// email and a desk that cannot describe a door two ways.
import { cleanLineColour, lineDimensions, lineExtras, lineMaterial, lineType } from "./pcd-line-summary";
...
function quoteLineRows(lines = [], { heading = "Requested items" } = {}) {
  if (!lines.length) {
    return `
          ...
          ${lines
            .map(
              (line) => `
                ...
                </tr>${lineExtras(line).length ? `
                <tr>
                  <td colspan="6" style="padding:0 8px 10px;color:#64748b;font-size:12px;line-height:17px;mso-line-height-rule:exactly;">${escapeHtml(lineExtras(line).join(" · "))}</td>
                </tr>` : ""}`
            )
            .join("")}
        </tbody>
...
export function quoteFacts(items, { emphasiseLast = false } = {}) {
  const cells = items
    ...
    .map(([label, value], index, all) => {
      ...
      return `
        <tr>
          <td style="padding:12px 16px;${divider}color:#7c725f;font-family:Arial,Helvetica,sans-serif;font-size:12px;line-height:18px;mso-line-height-rule:exactly;text-transform:uppercase;letter-spacing:1px;font-weight:700;">${escapeHtml(label)}</td>
          <td style="padding:12px 16px;${divider}color:#001f36;font-family:Arial,Helvetica,sans-serif;font-size:${last ? "16px" : "14px"};line-height:${last ? "22px" : "18px"};mso-line-height-rule:exactly;font-weight:${last ? "800" : "700"};text-align:right;">${escapeHtml(value)}</td>
        </tr>`;
    })
    .join("");
 ...
 */
export function customerUpdateHtml({ body }) {
  const text = String(body || "");

  // The orders this email is about, taken from the headings in the body rather
  // than passed in beside it, so the footer can only ever name what the person
  // actually read and approved in the box.
  const orderNumbers = [...new Set(text.match(/^PCD-O-[\w-]+/gm) || [])];
  ...
  const blocks = text
    ...
    .map((line, index, all) => {
      ...
      if (!row) return "";
      if (indented) {
        return `<div style="margin:0 0 5px;padding-left:14px;color:#263226;font-size:14px;line-height:1.55;border-left:2px solid #d5e4d1;">${escapeHtml(row.replace(/^-\s*/, ""))}</div>`;
      }
      ...
      if (/^PCD-O-/.test(row)) {
        return `<div style="margin:18px 0 8px;color:#001f36;font-size:13px;font-weight:700;letter-spacing:.5px;text-transform:uppercase;">${escapeHtml(row)}</div>`;
      }
      // The sign off is two lines of one thing, so they sit together rather
      // than a paragraph apart. Anything with a line under it and nothing
      // between them is the same sentence carrying on.
      const runsOn = (all[index + 1] || "").trim() !== "" && !/^\s{2,}/.test(all[index + 1] || "");
      return `<p style="margin:0 0 ${runsOn ? 0 : 14}px;color:#263226;font-size:15px;line-height:1.6;">${escapeHtml(row)}</p>`;
    })
    .join("");

  return quoteShell({
    title: "An update on your order",
    // The order number, not a reason for the email. The customer already knows
    // why we are writing; what they need at the bottom is the reference to
    // quote back at us if they forward it or ring about it.
    footerNote: orderNumbers.length
      ? `Sent regarding ${orderNumbers.length > 1 ? "orders" : "order"} ${orderNumbers.join(", ")}.`
      : "",
    children: `\n                ${blocks}`,
  });
}
...
// scheduleSentence is what the quote's suggested dates come to now that the
// customer has answered: the dates as a booking, or the fact that they lapsed.
// Written by acceptedScheduleSentence so this email and the quote page cannot
// end up describing the same window two different ways. Blank on a quote that
// suggested no dates, and the line is then simply not there.
export function customerQuoteApprovedHtml({ customerName, quoteNumber, orderNumber, scheduleSentence = "" }) {
  return quoteShell({
...
/**
 * A web order, paid for. The one confirmation that DOES list what they bought:
 * there was no quote to check it against, so this email is their only record of
 * the sizes, the edges and the drilling until the tax invoice arrives. Same line
 * table the quote request confirmation uses, reading the order's own lines.
 */
export function customerWebOrderHtml({ customerName, orderNumber, money, deliverTo, lines = [] }) {
  return quoteShell({
    title: "Thanks for your order",
    footerNote: `This email was sent because an order was paid for on our website${orderNumber ? `, ${orderNumber}` : ""}.`,
    children: [
      quoteParagraphs(
        [
          `Hi ${customerName || "there"},`,
          "Thanks, your payment has gone through and your order is with us. It goes on the bench in about ten working days, and we deliver it two to three days after it is made. If anything about it needs a word first, we will email you.",
        ].join(NEWLINE)
      ),
      quoteFacts(
        [
          orderNumber ? ["Order", orderNumber] : null,
          deliverTo ? ["Delivering to", deliverTo] : null,
          ["Paid", money],
        ],
        { emphasiseLast: true }
      ),
      quoteLineRows(lines, { heading: "What we are making" }),
      quoteParagraphs(
...
export function customerQuoteRequestHtml(payload) {
  return quoteShell({
    ...
    children: [
      ...
      quoteFacts([
        payload.productName ? ["Product", payload.productName] : null,
        ["Items", String(payload.lines?.length || 0)],
      ]),
      quoteLineRows(payload.lines || []),
    ].join(""),
  });
...
//
// Four emails. The two a customer sees are on the cream shell, because it is
// the wrapper they already associate with us. The two we see are on the navy
// one, so an internal notice can never be mistaken for something a customer
// sent.
//
// See lib/pcd-booking-confirmations.js for when each of these goes out.

/** The ask, a day before the visit. */
export function customerBookingAskHtml({
  customerName,
  kindLabel,
  dayLong,
  dayShort,
  ...
  const kind = String(kindLabel || "booking").toLowerCase();
  // "at 9:30am" for a timed visit, nothing at all for an all day one, because
  // an all day booking has no start time worth printing.
  const at = startTime ? `, at ${startTime}` : "";

  return quoteShell({
    title: `Please confirm your ${kind} tomorrow`,
    // SAYS IT IS AUTOMATIC AND THAT A REPLY IS READ, for the same reason the
    // quote expiry reminder does. A booking with something wrong in it is
    // exactly the sort of thing somebody replies to rather than clicks.
    footerNote:
      "This is an automatic reminder from our job calendar, sent the day before your booking. " +
      "Replies come straight through to our team and are read by a person.",
    children: [
      quoteParagraphs(
        [
          `Hi ${customerName || "there"},`,
          `We have a ${kind} booked for you tomorrow, ${dayShort}${at}. Please confirm it still suits you ` +
            `so we know to head out.`,
        ].join(NEWLINE)
      ),
      quoteFacts([
        ["What", kindLabel],
        ["When", dayLong],
        ["Time", timeRange],
        ["Where", siteAddress || "We do not have an address yet"],
      ]),
      // Warned before they click, and only about what is actually short. A
      // customer who gave us a number last month must not be told we have none.
      missing.length
        ? quoteParagraphs(
            `We are also missing ${missing.join(" and ")} for this one. The page will ask you for ` +
              `${missing.length > 1 ? "those" : "that"} before you can confirm, because we cannot turn up ` +
              `without ${missing.length > 1 ? "them" : "it"}.`
          )
        : "",
      quoteButton(confirmUrl, "Confirm this appointment"),
      fallbackLink(confirmUrl),
      quoteParagraphs(
        [
          `If tomorrow no longer suits, use the same link to tell us. You do not need to give a reason. If you ` +
            `would like another time, say so in the box on that page and we will sort it out.`,
          `If anything above is wrong, reply to this email or give us a ring on ${BUSINESS_PHONE} and we will ` +
            `fix it before we come out.`,
        ].join(NEWLINE)
      ),
    ].join(""),
  });
...
/** Their receipt, once they answer. Short, and it promises nothing. */
export function customerBookingAnsweredHtml({
  customerName,
  kindLabel,
  dayLong,
  dayShort,
  ...
  if (confirmed) {
    return quoteShell({
      title: `Your ${kind} is confirmed`,
      footerNote:
        "Sent because you answered a booking confirmation for us. Replies come straight through to our team " +
        "and are read by a person.",
      children: [
        quoteParagraphs(
          [
            `Hi ${customerName || "there"},`,
            `Thanks for confirming. Your ${kind} is booked in for ${dayShort}${at} and we will see you then.`,
          ].join(NEWLINE)
        ),
        quoteFacts([
          ["What", kindLabel],
          ["When", dayLong],
          ["Time", timeRange],
          ["Where", siteAddress || "Not recorded"],
          ["Best contact", contact || "Not recorded"],
        ]),
        supplied.length
          ? quoteParagraphs(
              `We have saved the ${supplied.join(" and ")} you gave us against your job, so we have ` +
                `${supplied.length > 1 ? "them" : "it"} for next time as well.`
            )
          : "",
        quoteParagraphs(
          [
            "If anything changes before then, reply to this email or give us a ring and we will move it.",
  ...
  // A CANCELLATION MUST NOT READ AS A CONFIRMATION.
  //
  // This carried the same tidy panel of times as the message above, with a
  // sentence or two saying the visit was off. Skimmed, that is a booking
  // somebody believes is still on. The word now sits in the subject, the
  // heading, the strip above the message and the first row of the panel, and
  // the times underneath are struck through.
  //
  // Said once in each place and left there. It is a cancellation the customer
  // asked for, not news that needs pressing on them.
  //
  // The date stays, because somebody with a measure and a delivery in the same
  // week has to know which of them this is about.
  //
  // AND IT PROMISES NOTHING. It does not say we will ring them. Somebody who
  // has gone elsewhere should not be told to expect a call.
  return quoteShell({
    title: `Cancelled: your ${kind} on ${dayShort}`,
    footerNote:
      "Sent because you answered a booking confirmation for us. Replies come straight through to our team " +
      "and are read by a person.",
    children: [
      `<p style="margin:0 0 16px;border:1px solid #e6b4a9;border-left:4px solid #b42318;background:#fdf4f2;` +
        `color:#8a2d1c;padding:12px 15px;font-size:12px;line-height:1.4;font-weight:700;letter-spacing:1px;` +
        `text-transform:uppercase;">Cancelled` +
        `<span style="display:block;margin-top:5px;font-size:15px;font-weight:400;letter-spacing:0;` +
        `text-transform:none;color:#7a4038;">Your ${escapeHtml(kind)} on ${escapeHtml(dayShort)} is not ` +
        `going ahead.</span></p>`,
      quoteParagraphs(
        [
          `Hi ${customerName || "there"},`,
          `Thanks for letting us know that ${dayShort} does not suit. We have taken the ${kind} off our run ` +
            `for that day and there is nothing else you need to do.`,
        ].join(NEWLINE)
      ),
      cancelledFacts([
        ["Status", "Cancelled"],
      ...
      notes ? quoteParagraphs(`You told us: ${notes}`) : "",
      quoteParagraphs(
        "If you would like to arrange another time, or there is anything else we can help you with, reply to " +
          "this email or give us a call and we will sort it out."
      ),
    ].join(""),
  });
}
...
/**
 * The cream facts panel, struck through.
 *
 * Its own function rather than a flag on quoteFacts, because every other use of
 * that panel is a live booking and none of them should be one edit away from
 * rendering as a cancellation.
 */
function cancelledFacts(items) {
  const cells = items
    .filter(Boolean)
    .map(([label, value], index) => {
      const divider = index === 0 ? "" : "border-top:1px solid #e3cdc8;";
      const valueStyle =
        index === 0
          ? "color:#8a2d1c;font-weight:800;"
          : "color:#8b7a76;font-weight:700;text-decoration:line-through;";
      const labelColour = index === 0 ? "#8a2d1c" : "#9c8a86";
      return `
        <tr>
          <td style="padding:14px 16px;${divider}color:${labelColour};font-size:12px;text-transform:uppercase;letter-spacing:1px;font-weight:700;">${escapeHtml(label)}</td>
          <td style="padding:14px 16px;${divider}${valueStyle}font-size:14px;text-align:right;">${escapeHtml(value)}</td>
        </tr>`;
    })
    .join("");

  return `<table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin:20px 0;border-collapse:collapse;background:#fbf7f6;border:1px solid #e3cdc8;">${cells}</table>`;
}

/**
 * A notice to sales@ about one booking.
 *
 * Used for all three internal cases: a booking of ours with nobody on it, an
 * answer that has just come in, and an ask that could not be sent.
 *
 * FIGURES, NOT A VERDICT. It lists what is on the booking and what is blank. It
 * does not say the appointment is good to go, because whether it is depends on
 * things the calendar knows nothing about.
 */
export function salesBookingNoticeHtml({
  heading,
  intro,
  rows: detail = [],
  missing = [],
  ...
  const completeness = missing.length
    ? `<div style="margin:22px 0 8px;color:#1d3a24;font-size:12px;line-height:16px;letter-spacing:1.2px;text-transform:uppercase;font-weight:700;">Still missing</div>
       <div style="padding:13px 16px;border:1px solid #e8d68f;border-radius:8px;background:#fffdf0;color:#7a6109;font-size:13px;line-height:20px;">${escapeHtml(
         missing.join(", ").replace(/, ([^,]*)$/, " and $1")
       )}. Nothing here stops the visit going ahead, it is what we do not hold.</div>`
    : `<div style="margin:22px 0 8px;color:#1d3a24;font-size:12px;line-height:16px;letter-spacing:1.2px;text-transform:uppercase;font-weight:700;">What we hold for this visit</div>
       <div style="padding:13px 16px;border:1px solid #cde0c8;border-radius:8px;background:#f4faf3;color:#24491f;font-size:13px;line-height:20px;">Address, contact name and mobile number are all on the booking.</div>`;

  const added = supplied.length
    ? `<div style="margin:22px 0 8px;color:#1d3a24;font-size:12px;line-height:16px;letter-spacing:1.2px;text-transform:uppercase;font-weight:700;">Added by the customer just now</div>
       <div style="padding:13px 16px;border:1px solid #e2e8f0;border-radius:8px;background:#f8fafc;color:#334155;font-size:13px;line-height:20px;">
         <ul style="margin:0 0 6px;padding-left:18px;">${supplied
           .map((line) => `<li>${escapeHtml(line)}</li>`)
           .join("")}</ul>
         ${supplied.length > 1 ? "These are" : "This is"} now on the booking and on the Outlook event. The customer record has taken ${
           supplied.length > 1 ? "them" : "it"
         } wherever that field was blank.
       </div>`
    : "";
  ...
  const button = actionUrl
    ? `<p style="margin:18px 0 0;"><a href="${escapeHtml(
        actionUrl
      )}" style="display:inline-block;background:#0d3550;color:#ffffff;text-decoration:none;padding:12px 18px;font-size:13px;font-weight:700;border-radius:6px;">${escapeHtml(
        actionLabel || "Open it"
      )}</a></p>`
    : "";
...
/**
 * The morning list: what is on today that nobody has answered.
 *
 * The two sections are the whole point. "Asked and waiting" is a customer who
 * has not replied. "Could not be asked" is our problem, not theirs, and the two
 * need opposite responses from whoever opens this.
 */
export function salesBookingChaseHtml({ waiting = [], couldNotAsk = [], adminUrl }) {
  const table = (heading, items, empty) => {
    if (!items.length) {
      return `
      <div style="margin-top:22px;">
        <div style="margin:0 0 8px;color:#1d3a24;font-size:12px;line-height:16px;letter-spacing:1.2px;text-transform:uppercase;font-weight:700;">${escapeHtml(heading)}</div>
        <div style="padding:14px 16px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;color:#64748b;font-size:13px;line-height:20px;">${escapeHtml(empty)}</div>
      </div>`;
    }
    return `
      <div style="margin-top:22px;">
          ...
          <tbody>
            ${items
              .map(
                (item) => `
              <tr>
                <td style="padding:10px 12px;border-top:1px solid #e2e8f0;color:#0f172a;font-size:13px;line-height:19px;">
                  <b>${escapeHtml(item.title)}</b><br />
                  <span style="color:#64748b;">${escapeHtml(item.detail)}</span>
                </td>
              </tr>`
              )
              .join("")}
  ...
  return emailShell({
    title: "Bookings today nobody has answered",
    intro:
      "Everything on the calendar for today that was asked about and has not come back, and anything we could " +
      "not ask about at all.",
    children: `
      ${table("Asked, still waiting", waiting, "None. Everything on today has been answered.")}
      ${table("Could not be asked", couldNotAsk, "None. Every ask went out.")}
      ${
        adminUrl
