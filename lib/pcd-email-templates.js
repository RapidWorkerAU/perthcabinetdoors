...
// The closing line all three share, so a change of tone happens once.
const CONFIRMATION_SIGN_OFF = `
      <p style="margin:0;color:#334155;font-size:15px;line-height:23px;">
...
export function customerPaymentReceivedHtml({ customerName, money, orderNumber }) {
  return emailShell({
    ...
    intro: `Hi ${customerName || "there"}, thanks, we have received your payment of ${money}${
      orderNumber ? ` for order ${orderNumber}` : ""
    }.`,
    children: CONFIRMATION_SIGN_OFF,
  });
}
...
export function customerQuoteApprovedHtml({ customerName, quoteNumber, orderNumber }) {
  return emailShell({
    ...
    intro: `Hi ${customerName || "there"}, thanks for approving ${
      quoteNumber ? `quote ${quoteNumber}` : "your quote"
...
export function customerVariationApprovedHtml({ customerName, variationNumber, orderNumber }) {
  return emailShell({
    ...
    intro: `Hi ${customerName || "there"}, thanks for approving ${
      variationNumber ? `variation ${variationNumber}` : "the change"
