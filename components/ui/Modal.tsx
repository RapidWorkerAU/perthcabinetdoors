        ...
        */}
        <Dialog.Content
          className={cn(
            ...
            !isSheet && 'inset-0 rounded-none flex flex-col',
            !isSheet && 'md:inset-auto md:left-1/2 md:top-1/2 md:-translate-x-1/2 md:-translate-y-1/2',
            !isSheet && 'md:rounded-[8px] md:max-h-[90vh] md:flex md:flex-col',

            // CONFIRM MODAL — mobile: bottom sheet, wraps content
            isSheet && 'inset-x-0 bottom-0 rounded-t-[16px] rounded-b-none',
            isSheet && 'md:inset-auto md:left-1/2 md:top-1/2 md:-translate-x-1/2 md:-translate-y-1/2',
            isSheet && 'md:rounded-[8px]',
        ...
        >
          ...
          {/* ── Desktop header ──────────────────────────────────────────── */}
          {!hideHeader && (
            <div className="hidden md:flex items-start justify-between px-6 pt-5 pb-4 flex-shrink-0 border-b border-[#edf4eb]">
              <div>
                <Dialog.Title className="text-[16px] font-semibold text-[#1a1a18] leading-snug">
                  {title}
                </Dialog.Title>
                {subtitle && (
                  <Dialog.Description className="text-[13px] text-[#5a5a52] mt-[3px]">
                    {subtitle}
                  </Dialog.Description>
                )}
              </div>
              {!hideCloseButton && (
                <button
                  ...
                  onClick={onClose}
                  aria-label="Close"
                  className="w-[28px] h-[28px] rounded-[6px] flex items-center justify-center text-[#8b8a81] hover:bg-[#edf4eb] hover:text-[#1a1a18] transition-colors flex-shrink-0 -mt-[2px]"
                >
                  <IconX size={15} />
                </button>
              )}
          ...
          {/* ── Mobile header ───────────────────────────────────────────── */}
          {!hideHeader && (
            <div className="flex md:hidden items-center gap-3 px-4 pt-4 pb-3 flex-shrink-0 border-b border-[#edf4eb]">
              <button
                type="button"
                onClick={onClose}
                aria-label="Go back"
                className="w-[28px] h-[28px] rounded-[6px] flex items-center justify-center text-[#8b8a81] hover:bg-[#edf4eb] hover:text-[#1a1a18] transition-colors flex-shrink-0"
              >
                <IconArrowLeft size={18} />
              </button>
              <span className="flex-1 text-center text-[15px] font-semibold text-[#1a1a18] leading-snug">
                {title}
              </span>
          ...
          */}
          <div className={cn(
            ...
            !isSheet && 'flex-1 overflow-y-auto flex flex-col gap-4',
            isSheet  && 'overflow-visible',
          )}>
            {children}
          </div>
          ...
          {footer && (
            <div className={cn(
              'flex-shrink-0 border-t border-[#edf4eb]',
...
export function ConfirmModal({
  ...
  const iconEl =
    variant === 'danger'  ? <IconTrash        size={20} className="text-[#b42318]" /> :
    variant === 'warning' ? <IconAlertTriangle size={20} className="text-[#f59e0b]" /> :
                            <IconHelp          size={20} className="text-[#5a5a52]"  />
  ...
  return (
    ...
    >
      ...
      <div
        className="flex md:hidden mx-auto w-[36px] h-[4px] bg-[#dbd8cc] rounded-full mb-2"
        aria-hidden="true"
      />
      ...
      <div className="flex flex-col items-center text-center px-2 pb-2">
        ...
        <Dialog.Title className="text-[15px] font-semibold text-[#1a1a18]">
          {title}
        </Dialog.Title>
        <p className="text-[14px] text-[#5a5a52] leading-relaxed mt-2">
          {description}
        </p>
      </div>
    </Modal>
